...
# 要件定義書（`docs/requirements/`）と ADR（`docs/adr/`）の記述ルール

要件定義書と ADR を編集するときに従う規約。

要件定義書は **5 バケット時系列構造**（[ADR 0001](../../docs/adr/0001-requirements-as-5-buckets.md)）で運用する。
...
## 3. 要件定義書（`docs/requirements/`）のディレクトリ構造と守備範囲
...
| `03-page-routing.md` | 画面ルーティングと認証ガード（全画面 × 認証状態の着地マトリクス・ガード方法の使い分け・グローバルナビ） |
...
- **配置**：フラット（サブディレクトリなし）。1 機能 1 ファイル
- **ファイル名**：`<kebab-case-name>.md`（数値 ID は採番しない）
  - **1 ドメイン 1 ファイル**：例 `authentication.md` / `grading.md` / `learning.md`
  - **1 ドメインに複数ワークフロー**：ドメイン名 prefix で分割し、alphabetical sort で並ぶようにする（例：`problem-generation.md` / `problem-display-and-answer.md`）
...
- 10+ ドメインに広がった段階でカテゴリ別サブディレクトリ復活を検討（現状はフラットで十分、prefix で同ドメインが並ぶため）
...
- **ファイル長は許容する（行数で分割しない）**：分割トリガはあくまで **ドメイン境界**。同一ドメインなら 600 行・1000 行になっても 1 ファイルに保つ。理由：(1) path が安定参照子なので外部リンクを切らない、(2) 「どっちのファイルに書くか」の判断コストを永続的に発生させない、(3) LLM 駆動開発では 1 ファイル完結のほうが context として精度が出る。長さに対しては以下で対処する：
  - **ビジネスルールが 30 行を超えたら H3 サブセクションに割る**（壁になるのを防ぐ）
  - **冒頭の「機能一覧」テーブルを目次として活用する**（長いファイルへの入口）
  - ドメイン横断が始まった時のみ prefix で別ファイルに分ける（例：`authentication.md` が課金まで抱えたら `billing.md` を切り出す）
...
- **4-features/`<name>.md`**：機能単位のユーザーストーリー + 受け入れ条件 + 画面 + API + フロー（個別機能の SSoT）
...
ファイルの種類によって冒頭の書き方を分ける。

### 7-A. 参照系ドキュメント（1-vision / 2-foundation / 3-cross-cutting / 4-features / ADR 等）
...
### 7-B. フェーズ系ドキュメント（`5-roadmap/r0-setup/<phase>.md` 等、手順書）

- ファイルの先頭（H1 直下）に **`## このフェーズで何ができるようになるか`** の H2 セクションを置き、本フェーズ完了時にできるようになることを箇条書きで列挙する
- 守備範囲を別に書くと重複するため、**守備範囲は書かない**（できるようになることを書けば守備範囲も自然に伝わる）
- 前提フェーズ / 次フェーズ / 共通ポリシー等のメタ情報は、H2 セクションの下の `---` 区切りに続けて引用ブロックで書く
- 例：
...
<1〜2 文の概要>。本フェーズが終わると以下ができるようになる：
...
> **前提フェーズ**：[xxx.md](./xxx.md) 完了済
> **次フェーズ**：[yyy.md](./yyy.md)
```

---

## 8. 図の表現方法（Mermaid と ASCII を内容で使い分け）

要件定義書では **GitHub ネイティブサポートの Mermaid 図** を基本に、内容に応じて **テキスト擬似図（ASCII アート）** も選択する。「Mermaid > ASCII」と機械的に決めず、**伝えたい情報に対してどちらが読みやすいか**で判断する。
...
### ASCII（テキスト擬似図）が向くケース

| 種類 | 理由 |
|---|---|
| トレースのスパンツリー（インデント + ms 表示） | Jaeger/Tempo UI 等の業界標準テキスト表現が読みやすい |
| ディレクトリツリー（`├── / └──`）| ファイル配置はインデント構造の方が直感的、Mermaid 化するとノイズ |
| 短い線形フロー（3〜4 ステップ程度の `A → B → C`） | Mermaid 化するとボックスとレイアウト計算でかえって読みにくい |
| パイプ処理・データフローの 1 行表現（`Pydantic → OpenAPI → TS 型`） | 1 行で読める情報を図にする必要がない |
| コード例・JSON 例・SQL 例 | コードブロックで十分 |
| マイグレーション SQL の例示 | コードブロックで十分 |

### 判断指針

- **時系列 / 並行 / actor 間メッセージ**が主役 → Mermaid（特に `sequenceDiagram`）
- **構造 / 階層 / 配置**が主役で時間軸が無い → ASCII の方が速く読める
- **分岐が 3+ ある複雑なフロー** → Mermaid `flowchart`
- **線形 3〜4 ステップで終わる** → 番号付き箇条書き or 1 行 ASCII で足りる
- **同じ内容を両方で書ける場合** → 視覚的訴求力（採用担当者へのアピール）を優先するなら Mermaid、認知負荷を減らすなら ASCII
...
- [ ] ファイル冒頭の書き出しが §7 に従っているか（参照系：守備範囲 quote ブロック / フェーズ系：`## このフェーズで何ができるようになるか` H2 セクション）
- [ ] 章番号（`# NN.`）が **ファイル名の連番**と一致しているか
- [ ] 他ファイルへのリンクが正しい相対パスで書かれているか（GitHub レンダリングで切れない）
- [ ] フロー / 関係図 / 構造図 を **Mermaid / ASCII のどちらが読みやすいか** §8 の判断指針で検討したか
- [ ] Mermaid 図を追加・編集した場合、§8 アンチパターン（`<br/>` / dotted-arrow のドット / `->>-` 暗黙修飾子 / 特殊文字）に該当していないか
