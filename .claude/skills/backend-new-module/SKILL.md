...
> **本プロジェクトでは Repository レイヤを採用する**（[ADR 0044](../../../docs/adr/0044-backend-repository-pattern-adoption.md) が SSoT）。Router → Service → Repository → ORM の 3 層分離。本 SKILL で生成するのは router / schema / service / repository / model の 5 つ。Service は `AsyncSession` を `__init__` で受け取り、対応する Repository を保持してビジネスロジック + 認可 + Pydantic 詰め替えに専念。SQLAlchemy クエリの実装は Repository に集約する。**Service 単層構成（Repository 不採用）のテンプレートは別プロジェクトで参考にするため §付録 A にコメントとして残してある**（本 SKILL の能動指示は 3 層構成）。
...
├── repositories/$ARGUMENTS.py   # Repository（SQLAlchemy クエリ集約、ORM オブジェクトを返す、ADR 0044）
├── services/$ARGUMENTS.py       # Service（Repository を呼ぶ、ビジネスロジック + 認可 + Pydantic 詰め替え）
...
#### `repositories/$ARGUMENTS.py`（SQLAlchemy クエリの実装集約、ADR 0044）
...
class <Args>Repository:
    ...
    async def create(self, payload: <Args>Create, *, owner_id: UUID) -> <Args>:
        obj = <Args>(name=payload.name, owner_id=owner_id)
        self.session.add(obj)
        await self.session.flush()
        return obj

    async def get_by_id(self, id_: UUID) -> <Args> | None:
        stmt = select(<Args>).where(<Args>.id == id_)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
```

#### `services/$ARGUMENTS.py`（Repository を持つ 3 層構成、ADR 0044）
...
## 付録 A: Service 単層構成のテンプレート（参考・別プロジェクト用）

> **本プロジェクトでは採用しない**。以下のテンプレートは Repository レイヤを採用せず Service が `AsyncSession` から SQLAlchemy 2.0 を直接呼ぶ単層構成を採る別プロジェクト（例：薄い CRUD ばかりで Repository の delegating wrapper コストが見合わない / ROI 最大化が最上位目標 / 学習用途でなく本番効率を優先）で参照するために残してある。本プロジェクトでこの構成を生成しないこと（本プロジェクトの方針は [ADR 0044](../../../docs/adr/0044-backend-repository-pattern-adoption.md)）。

<!--
### 作成するファイル（Service 単層構成）
...
### 準拠するルール（Service 単層構成）

- **Service**：`AsyncSession` を `__init__` で受け取り、SQLAlchemy 2.0 のクエリを直接実行。トランザクション境界は Service が握る（`async with session.begin():`）。Pydantic への詰め替えも Service 内で行う
- **複雑なクエリ**：複数 Service で重複し始めたら `app/queries/<feature>.py` の関数群（クラス化はしない）に段階的に切り出す
-->
