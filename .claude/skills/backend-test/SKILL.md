...
手順 2 の方針提示で**ユーザーと対話的に確定した決定**を、テスト生成に入る前に要件 .md に反映する。テストの観測対象を要件側に揃えることで、要件・コード・テストの 3 者を SSoT として一貫させる。
