...
| E2E | `mise run e2e:up`（`docker-compose.e2e.yml`、tmpfs） | `localhost:5433` | `ai_coding_drill_e2e` | `mise run web:e2e` / CI の `web-e2e` ジョブ |

- ユーザー：`postgres`（両方とも、ローカル既定、本番は別途）
- 接続コマンド（dev）：`docker compose exec postgres psql -U postgres ai_coding_drill`
- 接続コマンド（E2E）：`docker compose -f docker-compose.e2e.yml exec postgres-e2e psql -U postgres ai_coding_drill_e2e`
- 接続文字列例（`.env` の `DATABASE_URL`、SQLAlchemy async + asyncpg）：
  ...
  - E2E：`postgresql+asyncpg://postgres:postgres@localhost:5433/ai_coding_drill_e2e`
- 破壊的テストエンドポイント（`/_test/reset`）の安全ガード：DB 名末尾 `_e2e` を要求する allowlist 方式。**dev DB へは構造的に届かない**（実装は `apps/web/e2e/_mock-github/server.py:_ensure_e2e_db_url`）
...
| E2E | `localhost:6380` | `docker compose -f docker-compose.e2e.yml exec redis-e2e redis-cli` |
...
- **SSoT 配置**：mise.toml（言語）/ pyproject.toml + uv.lock（Python 依存）/ package.json + pnpm-lock.yaml（Frontend 依存）/ go.mod + go.sum（Go 依存）/ docker-compose.yml（dev ミドルウェア）/ docker-compose.e2e.yml（E2E 専用ミドルウェア、issue #86）/ README.md（サマリ）
