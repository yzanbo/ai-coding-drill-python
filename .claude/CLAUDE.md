...
dev / E2E は **物理的に分離した 2 セット**（issue #86 の長期 fix）。dev:all と `web:e2e` を並行起動できる。

| 区別 | 起動 | ホスト | DB 名 | 用途 |
|---|---|---|---|---|
| dev | `docker compose up -d`（`docker-compose.yml`） | `localhost:5432` | `ai_coding_drill` | `mise run dev:all` / `api:dev` / `worker:grading:dev` |
| E2E | `mise run test:up`（`docker-compose.test.yml`、tmpfs） | `localhost:5433` | `ai_coding_drill_test` | `mise run web:e2e` / CI の `web-e2e` ジョブ |

- ユーザー：`postgres`（両方とも、ローカル既定、本番は別途）
- 接続コマンド（dev）：`docker compose exec postgres psql -U postgres ai_coding_drill`
- 接続コマンド（E2E）：`docker compose -f docker-compose.test.yml exec postgres-test psql -U postgres ai_coding_drill_test`
- 接続文字列例（`.env` の `DATABASE_URL`、SQLAlchemy async + asyncpg）：
  - dev：`postgresql+asyncpg://postgres:postgres@localhost:5432/ai_coding_drill`
  - E2E：`postgresql+asyncpg://postgres:postgres@localhost:5433/ai_coding_drill_test`
- 破壊的テストエンドポイント（`/_test/reset`）の安全ガード：DB 名末尾 `_test` を要求する allowlist 方式。**dev DB へは構造的に届かない**（実装は `apps/web/e2e/_mock-github/server.py:_ensure_test_db_url`）

### Redis 接続

dev / E2E でポートを分離する（DB index ではなくポートで分離する。FLUSHDB の巻き添えを構造的に避けるため）。

| 区別 | ホスト | 接続コマンド |
|---|---|---|
| dev | `localhost:6379` | `docker compose exec redis redis-cli` |
| E2E | `localhost:6380` | `docker compose -f docker-compose.test.yml exec redis-test redis-cli` |
...
**重要**：要件定義書と実装の整合性を常に取る（実装変更時は要件 .md も合わせて更新、ズレを残さない）。
...
- **SSoT 配置**：mise.toml（言語）/ pyproject.toml + uv.lock（Python 依存）/ package.json + pnpm-lock.yaml（Frontend 依存）/ go.mod + go.sum（Go 依存）/ docker-compose.yml（dev ミドルウェア）/ docker-compose.test.yml（テスト用ミドルウェア、issue #86）/ README.md（サマリ）
...
- **書かない場所**：名前で十分わかる箇所、変更経緯（`git log` で十分）、内部 ID やファイル参照（`R0-1 のため`等の陳腐化、ファイル名直書きの参照切れ）
