...
//   - POST /api/problems/generate に { category, difficulty } を投げて 202 + requestId を取る
...
import { requestProblemGenerationApiProblemsGeneratePost } from "@/__generated__/api/sdk.gen";
import type {
...
export const usePostProblemGenerate = (
  ...
  const mutation = useMutation<ProblemGenerateAcceptedResponse, ApiError, ProblemGenerateRequest>({
    mutationFn: (body) => throwIfError(requestProblemGenerationApiProblemsGeneratePost({ body })),
    onSuccess: (data) => {
