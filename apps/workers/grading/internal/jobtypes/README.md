...
`apps/api/app/schemas/` の Pydantic モデルから **自動生成された Go 構造体**を置く場所。Backend が enqueue するジョブペイロードを Worker が**同じ型**で受け取れるようにするための置き場（[ADR 0006](../../../../docs/adr/0006-json-schema-as-single-source-of-truth.md)）。
...
apps/api/job-schemas/<job_type>.json      (JSON Schema artifact)
  ↓ mise run worker:grading:types-gen
apps/workers/grading/internal/jobtypes/types.go   (Go struct、本ディレクトリ)
```

R0-7（型同期パイプライン基盤）+ R0-11（Worker 側合流）で完成。

## なぜ生成物を git に commit しないか

- 生成物は `.gitignore` で除外している（[本ディレクトリの .gitignore](./.gitignore)）
- CI で `mise run worker:grading:types-gen` を走らせて毎回再生成 → drift があれば CI 失敗で気づく
- 手で編集しても次の再生成で消える、レビューを通る前に既に古い、という事故を防ぐ
- 例外として本 README と `.gitignore` だけは commit してディレクトリ存在を残す

## やってはいけないこと

- **手書きで型を編集**：`mise run worker:grading:types-gen` で消える（[worker-layers.md §E §4](../../../../docs/requirements/5-roadmap/r0-setup/worker-layers.md)）
