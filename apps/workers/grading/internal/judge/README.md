...
「**LLM-as-a-Judge**（LLM に評価させる）」のロジックを置く package。grading worker では**受験者の解答品質**を評価し、generation worker では**生成された問題の品質**を評価する。`internal/llm/` の `Provider` interface を呼び出して使う側（[ADR 0008](../../../../docs/adr/0008-custom-llm-judge.md)）。

## 役割

- プロンプト YAML 読み込み：`apps/workers/grading/prompts/judge/*.yaml` から評価指示を取り込む（[ADR 0040](../../../../docs/adr/0040-worker-grouping-and-llm-in-worker.md)）
- 入力データを差し込み：`{{.solution}}` / `{{.test_results}}` 等のテンプレ変数を置換
...
- `llm/` は「API を叩く層」、`judge/` は「**grading 専用の prompt と response の整形層**」
- judge prompt の差し替えや評価軸の調整が `llm/` の provider 切り替えと独立にできる
- generation worker でも同名 package を持つが、prompt と評価軸が異なる（grading worker は解答評価、generation worker は問題評価）
...
- `internal/sandbox/` を import：同 Layer 1 内の横断は禁止、orchestrator（`internal/grading/`）経由にする（[worker-layers.md §E §2](../../../../docs/requirements/5-roadmap/r0-setup/worker-layers.md)）
...
- prompts：[apps/workers/grading/prompts/judge/](../../prompts/judge/)
