...
Postgres 接続の**インフラ層**だけを担当する package。具体的には `pgx` の **接続プール**を作って配り、**トランザクション**のヘルパを提供する。「問題生成ジョブの取得 SQL」や「結果書き戻しの INSERT」のような業務ロジックは置かない（→ そちらは `internal/job/` と `internal/generation/`）。
...
- 将来 `internal/problems/` のような「生成された問題を保存」専門の package を足すとき、`db/` を肥大化させずに `problems → db` の依存を 1 本足すだけで済む
