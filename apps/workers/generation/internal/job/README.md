...
- 将来 `internal/problems/` 等を追加するときも、`db/` 直下に新しい SQL を足すのではなく独立 package として横並びに置ける
...
- 生成 LLM 呼び出しは grading の judge LLM より時間がかかる場合がある（マルチ回呼び出しの場合は数十秒）。`locked_at` の更新は短いトランザクションで完了させ、重い処理は別トランザクションに分けるパターンを守る
- 失敗時の retry が「生成内容が違っても同じ問題として通る」べきか「同じプロンプトでもう一度 LLM に投げる」かは handler 側で判断（冪等性は handler 責務）
...
- 行ロックを長く握る：状態遷移を短いトランザクションで完了し、重い処理（生成 LLM 呼び出し・sandbox 検証）は別トランザクションに分ける（[02-architecture.md ジョブキュー](../../../../docs/requirements/2-foundation/02-architecture.md)）
