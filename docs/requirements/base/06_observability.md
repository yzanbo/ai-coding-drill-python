...
> **このドキュメントの守備範囲**：観測性（ログ・トレース・メトリクス・アラート）の設計、必須フィールド、データ保護方針、運用 Runbook。Phase 1 〜 Phase 4 を射程に含める。
> **採用するツール（Loki / Tempo / Prometheus / Grafana / Sentry 等）と選定理由**は [07_tech_stack.md: 観測性](./07_tech_stack.md#観測性) を参照。
...
- **必須フィールド**（Phase 1 から実装。**後追加だと過去ログを集計できない**ため最初から固める）：
...
- **LLM 呼び出し時の追加フィールド**（Phase 1 から記録、後追加だと履歴クエリ不可）：
...
- **本番**：ログ集約基盤に転送（採用先は [07: 観測性](./07_tech_stack.md#観測性)）
...
ログ・トレース・エラー追跡サービスへ送信する前に必ずマスキング・除外する。**Phase 1 から共通 redact ミドルウェアを導入**（後から PII を漏らした履歴を消す方が大変）：
...
- 採用 SDK・エクスポート先は [07: 観測性](./07_tech_stack.md#観測性) を参照
...
### プロセス境界をまたぐトレース連携（Phase 1 で必須）

NestJS（Producer）→ Postgres `jobs` → Go ワーカー（Consumer）はプロセスが分かれるため、**OTel Context をジョブペイロードに埋め込んで伝播させる**。これがないとユーザーリクエストとワーカー処理が別トレースに分断される。**ペイロードスキーマは Phase 1 で確定するため、この方針も Phase 1 で固める必要がある**。
...
→ ジョブペイロード JSON Schema にこのフィールドを必須として定義（[09_data_model.md: ジョブペイロードのスキーマ](./09_data_model.md#ジョブペイロードのスキーマ)）。
...
採用基盤は [07: 観測性](./07_tech_stack.md#観測性) を参照。Prometheus 形式でラベル付与し、Grafana で可視化する想定。Phase 4 でダッシュボード構築。
...
- **プロンプトバージョン別の成功率**（A/B テスト用、[05_llm_pipeline.md: プロンプト管理](./05_llm_pipeline.md#プロンプト管理) 参照）
...
[ADR 0001](../../adr/0001-postgres-as-job-queue.md) と [04_architecture.md: ジョブキュー](./04_architecture.md#ジョブキューpostgres-select-for-update-skip-locked) の運用作法を観測する：
...
- **レート制限ヒット数**（429 発生数、エンドポイント別。[03_non_functional.md: セキュリティ](./03_non_functional.md#セキュリティ最重要) の Sliding Log 動作確認）
- エラー率（`5xx` の比率）

### ヘルスチェック

[10_api_spec.md: ヘルスチェック・運用](./10_api_spec.md#ヘルスチェック運用) のエンドポイントを観測：

- `/healthz`（Liveness）失敗率
- `/readyz`（Readiness）失敗率と内訳（DB 接続失敗 / Redis 接続失敗）
- 連続失敗回数（再起動判断材料）

### ビジネス指標（Phase 4 以降で追加検討）
...
- 採用サービスは [07: 観測性](./07_tech_stack.md#観測性) を参照
...
| 事象 | 閾値（初期値） | 通知先 |
|---|---|---|
| LLM コストが日次予算を超過 | `> $30/day`（[03_non_functional.md](./03_non_functional.md#コスト) の上限） | メール |
...
- Slack / PagerDuty 連携は Phase 4 以降で必要になったら追加
...
`docs/runbook/` に以下の手順書を配置する（Phase 4 で順次整備）：
