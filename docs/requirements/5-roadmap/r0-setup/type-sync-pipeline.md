...
**設計判断**：
- **glob で SSoT 変更時のみ起動**：`apps/api/app/schemas/**` と `apps/api/app/routers/**` がステージング差分に含まれる時だけフックを走らせる。無関係な commit にコストを乗せない（→ [06-dev-workflow.md: drift 検出（lefthook + CI 二軸）](../../2-foundation/06-dev-workflow.md#drift-検出lefthook--ci-二軸)）
- **routers/ を含める理由**：path / method / docstring / response_model は OpenAPI の description・schema 構造に直接反映される。schemas/ だけを trigger 対象にすると、router の docstring 修正で artifact が drift しても pre-commit が skip され、CI の `git diff --exit-code` で初めて検出される事故が起きる（実際に発生 → routers/ を glob に追加して再発防止）
...
pre-commit:
  ...
  commands:
    ...
    # apps/api/app/schemas/ または app/routers/ 配下が変わったときに起動。
    # 3 artifact を再生成して差分が出たら abort し、開発者に
    # 「mise run api:openapi-export && mise run api:job-schemas-export &&
    # mise run web:types-gen して再 commit」を促す。
    #
    # 注意 1: glob は brace 展開で "apps/api/app/{schemas,routers}/**" と書く。
    #   lefthook の glob は string 必須（配列不可）のため、複数ディレクトリを
    #   含めるには brace 展開を使う。schemas/ だけだと routers/ の docstring /
    #   response_model 変更で artifact が drift しても hook が skip される事故が
    #   起きる（実発生済）。"schemas/**/*.py" だと schemas/ 直下のファイル
    #   （health.py 等）を lefthook の doublestar 実装が拾えないため "**" 単独にする。
    # 注意 2: run の先頭で set -e を必ず付ける。
    #   付けないと途中の mise run が失敗しても次のコマンドに進んでしまい、
    #   最後の git diff が「差分なし = pass」を返して誤って通る事象が発生する。
    schemas-drift:
      glob: "apps/api/app/{schemas,routers}/**"
      run: |
        set -e
