...
## 言語別 setup の後段：レイヤ分割フェーズ

各言語の setup が完了したあと、Backend / Frontend それぞれで**プロジェクト固有のレイヤ分割と import 方向**を確定するフェーズが続く。これは「悩む余地のない基本構造」と異なり、プロジェクトのドメイン特性を反映した設計判断を含むため、言語別環境構築とは別フェーズとして扱う。

具体的には：

- 該当言語の `app/` 配下にレイヤ別ディレクトリを `__init__.py`（Python の場合）または雛形ファイル（TS 等）付きで配置
- 各レイヤの責務・呼び出しの向き・命名規則を **`.claude/rules/{backend,frontend}.md`** に「実装契約」として固定（Claude が自動 load して読む契約ファイル）
