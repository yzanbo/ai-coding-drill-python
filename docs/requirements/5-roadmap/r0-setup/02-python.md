...
**作業内容**：
1. `cd apps/api && mise exec -- uv run alembic init alembic`
2. `apps/api/alembic.ini` の `sqlalchemy.url` を空にし、`apps/api/alembic/env.py` で `Settings().DATABASE_URL` から読み込むよう修正（同期 URL に変換するヘルパも仕込む）
3. `apps/api/alembic/env.py` の `target_metadata` を `app.db.base.Base.metadata` に差し替え
4. `mise exec -- uv run alembic revision --autogenerate -m "create health_check table"` で初版生成
5. `mise exec -- uv run alembic upgrade head` で適用
...
**目的**：FastAPI router → Service → SQLAlchemy session → Postgres の経路を end-to-end で 1 周通し、SQLAlchemy + Alembic + asyncpg 運用未経験箇所のつまずきを早期に洗い出す（リスクレジスタ対応）。

**作業内容**：
1. `apps/api/app/routers/health.py` に `POST /health` / `GET /health` を実装（`health_check` テーブルへ INSERT / SELECT）
2. `apps/api/app/main.py` で router を `include_router` に登録
3. `apps/api/tests/test_health.py` に integration テスト（`@pytest.mark.integration` 付き、httpx + ASGITransport で実通信、Postgres 必須）
...
mise exec -- uv run uvicorn app.main:app --reload &    # サーバ起動
...
**前提**：本ファイルの「8. mise.toml に Python タスク追記」
...
**前提**：本ファイルの「9. lefthook.yml に Python 用 pre-commit 追加」+「10. lefthook.yml に Python 用 pre-push 追加」（ローカル品質ゲートが緑）
