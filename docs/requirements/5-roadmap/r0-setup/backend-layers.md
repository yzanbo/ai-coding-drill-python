...
>
> **書き方の方針**：本フェーズは依存ライブラリのインストールを伴わないため、役割別フェーズの「環境構築 + 品質ゲート 5 ステップ」パターンには載らない。代わりに「**決定値の確認 → ディレクトリ作成 → 各レイヤ README → 全体 README → rules ファイルに実装契約として固定**」の構造で進める（Frontend レイヤ分割フェーズと同パターン）。
>
...
1. **ユーザーに設計上の確認を求めない**：`§決定値` に書かれた選択（9 レイヤ構成 / Repository 採用（Service / Repository / ORM の 3 層分離）/ admin・customer 分割不採用 / 命名規則 等）は**決定済みで議論しない**。関連 ADR（[ADR 0034](../../../adr/0034-fastapi-for-backend.md) / [ADR 0037](../../../adr/0037-sqlalchemy-alembic-for-database.md) / [ADR 0006](../../../adr/0006-json-schema-as-single-source-of-truth.md) / [ADR 0040](../../../adr/0040-worker-grouping-and-llm-in-worker.md) / [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）は既に消化済みのため、再検討しない。
...
```text
apps/api/app/
├── __init__.py                                      # backend.md 配置済
├── main.py                                          # FastAPI アプリ生成 + ルータ束ねる（backend.md 配置済）
├── README.md                                        # 全体図 + やってはいけないこと（本フェーズで作成）
├── core/                                            # 設定 / セキュリティ / 例外ハンドラ等の横断ユーティリティ（終端）
│   ├── __init__.py                                  # backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   └── config.py                                    # pydantic-settings ベースの Settings + get_settings()（backend.md 配置済）
├── db/                                              # SQLAlchemy エンジン / セッション / Base
│   ├── __init__.py                                  # backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   ├── base.py                                      # DeclarativeBase（backend.md 配置済）
│   └── session.py                                   # AsyncEngine + AsyncSessionLocal + get_async_session（backend.md 配置済）
├── models/                                          # SQLAlchemy モデル（1 テーブル 1 ファイル）
│   ├── __init__.py                                  # 全モデルを re-export（Alembic autogenerate が拾う）（backend.md 配置済）
│   ├── README.md                                    # 本フェーズで作成
│   └── health_check.py                              # 疎通確認用最小テーブル（backend.md 配置済）
├── schemas/                                         # Pydantic モデル（SSoT、HTTP API + Job キュー両境界に展開、ADR 0006）
│   ├── __init__.py                                  # backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   └── health.py                                    # HealthCheckResponse（backend.md 配置済）
├── routers/                                         # APIRouter（機能別 controller 相当、薄い受付係）
│   ├── __init__.py                                  # backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   ├── health.py                                    # POST/GET /health（DB 往復、backend.md 配置済）
│   └── probes.py                                    # GET /healthz（liveness、backend.md 配置済）
├── services/                                        # ビジネスロジック + 認可チェック + Pydantic 詰め替え（DB クエリは repositories/ に委譲、R1 以降で実体化、ADR 0044）
│   ├── __init__.py                                  # 本フェーズで作成（空ファイル）
│   └── README.md                                    # 本フェーズで作成
├── repositories/                                    # SQLAlchemy クエリ実装（ORM オブジェクトを返却、R1 以降で実体化、ADR 0044）
│   ├── __init__.py                                  # 本フェーズで作成（空ファイル）
│   └── README.md                                    # 本フェーズで作成
...
> **既存実装ファイル**（backend.md 配置済、本フェーズでは触らない）：`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `models/__init__.py`（`from app.models.health_check import HealthCheck` を re-export）/ `schemas/health.py` / `routers/health.py` / `routers/probes.py`。中身の SSoT は [backend.md](./backend.md) の各 step。
>
> **`services/` `repositories/` `deps/` `observability/` が空フォルダで残る理由**：R0 段階では業務機能（auth / problems 等）が未着手のため、これらレイヤに置く実装ファイルがまだ無い。R1 以降の機能追加に合わせて埋まる。R0 では `__init__.py` + `README.md` だけを置いて空フォルダ防止と意図の明示を兼ねる。
>
...
| Repository レイヤ | **採用**（Service → Repository → ORM の 3 層分離） | Service が `AsyncSession` から SQLAlchemy 2.0 を直接呼ぶ単層構成 / `app/queries/<feature>.py` 関数群の段階導入 | 本プロジェクトは ROI 最大化ではなく**ポートフォリオでの設計パターン理解の可視化**を最上位目標とする（→ [01-roadmap.md: ビジョン](../01-roadmap.md#ビジョン変わらない北極星)）。中〜大規模 SaaS の業界標準である Service / Repository / ORM 分離を最初から採用し、面接官が `apps/api/app/repositories/` の存在で設計意図を即読み取れる状態を作る。ROI 観点では Repository が delegating wrapper になる事実は受容するが、portfolio 価値・テスタビリティ（Service 単体テストで Repository モック可）・責務の明示的分離を ROI に優先する。`queries/` 段階導入は廃止し、最初から Repository に統一する（→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）|
...
| `services/` | `repositories` / 他の `services` / `schemas` / `models`（型注釈のみ）/ `core` | `routers` / `deps` / `main` / `db` を直接（DB セッションは Service の引数で受け取り、クエリは repositories 経由、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)） |
...
- **依存は一方向**：A → B かつ B → A を作らない。`services/` 内の機能間も同じ（例：`grading` → `submissions` の向きに揃え、逆向き import を作らない）。`repositories/` 内の機能間も同様
- **副作用は直接 import で繋がない**：通知・発火等は FastAPI の `BackgroundTasks` または domain event 経由で起動し、別 router / service を直接 import しない
- **`repositories/` が DB アクセスの唯一の入口**：Service は SQLAlchemy を直接呼ばず、Repository メソッドを呼ぶ。Repository は ORM オブジェクトを返し、Pydantic への詰め替えは Service が行う（→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
...
- **trivial 例外**：`health_check` のような INSERT 1 行 / SELECT 1 行レベルは Service / Repository を作らず router 直書きを許容（[backend.md: 7. 薄い CRUD エンドポイント](./backend.md#7-薄い-crud-エンドポイント--pytest-疎通) の設計判断に従う）
...
| クラス名（モデル / スキーマ / Service / Repository） | PascalCase | `HealthCheck` / `ProblemService` / `ProblemRepository` / `Settings` |
...
| Repository クラスの suffix | `<Feature>Repository` | `ProblemRepository` / `SubmissionRepository` |
...
10. `services/` 内で SQLAlchemy を直接呼ぶ（`select` / `insert` / `update` / `delete` + `await session.execute(...)`）。DB クエリは `repositories/` に切り出し、Service は Repository メソッドを呼ぶ（`§B`、Repository 採用、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
...
14. ソフトデリート対象テーブル（`users` / `problems` / `submissions`）のクエリで `WHERE deleted_at IS NULL` の付け忘れ（本人画面・所有権チェック系は必ず付ける。ただし統計・集計クエリでは意図的に付けない場合があるので呼び出し側で選択する、→ [ADR 0048](../../../adr/0048-soft-delete-for-user-facing-tables.md) / [.claude/rules/backend.md](../../../../.claude/rules/backend.md)）
...
**目的**：`apps/api/app/` 配下に Backend のレイヤを 1 つの方針で固定する。以降の機能実装はこの 9 フォルダのどこに置くかを判断するだけ、という状態を作る。

**最終状態**（§自律実行ポリシー §7 の通り、初期状態のばらつきは問わない）：

- `apps/api/app/` 配下が §A のツリーに一致している：
  - 9 レイヤすべて存在する：`routers/` / `services/` / `repositories/` / `schemas/` / `models/` / `core/` / `db/` / `deps/` / `observability/`
  - 各レイヤに `__init__.py` がある（Python パッケージとして認識させるため、空でも OK）
  - `main.py` が存在し、`include_router(probes.router)` / `include_router(health.router)` で backend.md 配置済の 2 router を登録している
- **計 10 個の `README.md` ファイル**が存在する：
  - `apps/api/app/README.md`（1 個、`app/` 直下）
  - 上記 9 レイヤそれぞれに `README.md`（9 個）
  - 中身は手順 2 / 手順 3 で作る。本手順 1 の段階では空ファイルでもよい（最終的に手順 2 / 3 で書き込めば OK）
- `.gitkeep` ファイルが残っていない（README が空フォルダ防止を兼ねるため、`.gitkeep` は不要）
- §A のツリーに無い別のフォルダ（例：`app/llm/` のような LLM 用、`app/utils/` のような曖昧な名前のフォルダ、`app/queries/` のような旧段階導入策、admin / customer の分割等）が残っていない

**backend.md 完了済を前提とした、本フェーズで実際に新規追加する差分**：

> 前提フェーズ（[backend.md](./backend.md)）完了時点で、`routers/` / `schemas/` / `models/` / `core/` / `db/` の 5 ディレクトリと、それらに属する実装ファイル（`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/__init__.py` + `models/health_check.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`）は既に配置済。**本フェーズはその上に「足りないレイヤ 4 つ」と「全レイヤの README + 実装契約」を載せるだけ**で、backend.md 由来の既存ファイルには手を入れない。
>
> | 差分カテゴリ | 中身 |
> |---|---|
> | **新規フォルダ（4 個）** | `services/` / `repositories/` / `deps/` / `observability/`（各 `__init__.py` を空ファイルで配置） |
> | **新規 README（10 個）** | `app/README.md` + 9 レイヤ分の `README.md`（backend.md 配置済の 5 レイヤにも README が無いので追加する。本フェーズ手順 2 / 3 で書き込む） |
> | **実装契約の追加 / 整合化** | `.claude/rules/backend.md` に §A ツリー + §C import 方向表 + §D 命名規則 + OK/NG コード片を追加（または既存セクションを `§決定値` に合わせて整合化）。詳細は手順 4 |
> | **触らないもの** | backend.md 配置済の `main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `models/__init__.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`、および `alembic/` / `tests/` / `pyproject.toml` / `uv.lock` / `.env.example` |

**完了基準**：

- 上記 10 個の README.md が（空でも可）存在する
- 9 レイヤすべてが空ではない（`__init__.py` + README 入り）
- backend.md 配置済の実装ファイル（`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`）に変更が入っていない
...
**最終状態で存在すべき README**（全 10 ファイル）：

| パス | 重視する内容 |
|---|---|
| `apps/api/app/README.md` | トップの全体図（手順 3 で詳述） |
| `apps/api/app/routers/README.md` | 「URL の受付係」の役目 / `Depends` で前処理を受け取る / 業務処理は services に丸投げ |
| `apps/api/app/services/README.md` | 「業務処理本体」の役目 / 認可チェック + ビジネスロジック + Pydantic 詰め替え / DB クエリは repositories に委譲 / `routers` を import 禁止 / 結果は `schemas` に詰め替えて返す |
| `apps/api/app/repositories/README.md` | 「DB アクセスの窓口」の役目 / SQLAlchemy クエリの実装集約 / 戻り値は ORM オブジェクト / Pydantic 変換は Service / ポートフォリオ駆動の採用根拠（→ ADR 0044）/ `services/` との違い対比表 |
...
| `apps/api/app/db/README.md` | `AsyncEngine`（接続プール）+ `AsyncSession`（1 リクエスト 1 つ）+ `Base`（モデル共通親）/ 実 SQL は repositories 側 |
...
| # | 項目 | 何のために（要点）| 根拠 |
|---|---|---|---|
| 1 | アプリ設定値の管理（`config.py`）| `.env` / 環境変数から読み込んだ設定値を型付きで一元管理（pydantic-settings ベースの `Settings` + `get_settings()`）| backend.md step 4 配置済 |
...
| # | 項目 | 何のために（要点）| 根拠 |
|---|---|---|---|
| 1 | DB セッションの貸し出し（`get_async_session`）| リクエスト開始時に開き、終了時に確実に閉じる処理を 1 箇所に集約。閉じ忘れによる接続プール枯渇を構造的に防ぐ | backend.md step 4 で配置済 / R1 で `deps/db.py` に移植 |
...
| 8 | ヘルスチェック（`/healthz` / `/readyz`）| k8s / ALB がインスタンスの受付可否を機械可読に判定、デプロイ時の無停止切替を支える | backend.md step 8 で `/healthz` 配置済、`/readyz` は R1 以降 |
...
- 上記 10 個の README.md が存在する
- 各 README は「とは何か」セクションを冒頭に持つ
- 紛らわしいレイヤ組（`core/` と `deps/`、`schemas/` と `models/`、`services/` と `repositories/`、`routers/health.py` と `routers/probes.py`）には対比的な記述がある
...
B. **ASCII 図でレイヤ間の呼び出し方向**を示す。`§C` の import 方向と一致する：
   ...
   - `schemas` は参照タイミングが 2 つあるため意図的に 2 度描く（右上 = routers がレスポンス形を宣言、右下 = services が Repository から受け取った ORM オブジェクトを Pydantic に詰め替え）
   - `repositories` は SQLAlchemy 実装を集約し、ORM オブジェクトを返す（→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
   - `core` / `observability` は別系統として図外に補足

C. **読み方の具体例**が含まれる：
   - 正常な流れ（`/problems` → `routers/problems.py` → `services/problems.py` → `repositories/problems.py` → `models/problems.py` → DB → Repository が ORM を返す → Service が `schemas/problems.py` の形に詰め替えて返す）
   ...
   - `services/` と `repositories/` の責務分担：Service はビジネスロジック + 認可 + トランザクション境界 + Pydantic 詰め替え、Repository は SQLAlchemy クエリ実装のみ（→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
...
- `§A` のツリー（コメント付き）
- 「Repository レイヤを採用（Service / Repository / ORM の 3 層分離、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）」「admin / customer 分割は採用しない」「横断的な部品は責務に近い場所」「LLM SDK は Backend に置かない」の 4 つの設計方針
...
C. **`### 設計方針` セクション**が「admin / customer 分割不採用」「`Depends` で認証要否切替」「横断部品は責務に近い場所」「Repository 採用（Service / Repository / ORM の 3 層分離、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）」の 4 点を含む
...
- AsyncEngine + AsyncSession、Service / Repository とも `async def`
- セッション取得：`Annotated[AsyncSession, Depends(get_async_session)]`（B008 違反回避）。Service が受け取り、Repository に渡す（→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
- タイムゾーン：`TIMESTAMP(timezone=True)` で UTC、表示は JST
- IDs：UUID（`gen_random_uuid()`）または BigInteger（`jobs.id` のみ）
- 削除方針：`users` / `problems` / `submissions` はソフトデリート（`deleted_at`）、`auth_providers` / `generation_requests` / `jobs` はハードデリート（→ [ADR 0048](../../../adr/0048-soft-delete-for-user-facing-tables.md)）
- where 条件の必須ルール（`user_id == current_user.id` / `len(ids) == 0` ガード、Repository メソッド側で実装）
...
I. **`### Router` / `### Pydantic スキーマ（SSoT）` / `### Service` / `### Repository` の 4 サブセクション**が `§D` の命名規則表 + 書き方ルール（`prefix` + `tags`、`response_model` 必須、`<Model>Create/Update/Response/Query` 命名、Service の `async with session.begin()` トランザクション境界、Repository が ORM オブジェクトを返し Service が `model_validate` で詰め替える等、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）を含む

J. **`## 新規機能の追加パターン` セクション**が「`/backend-new-module` スキル使用 or 手動で 5 ステップ（models / schemas / repositories / services / routers）+ `include_router` + Alembic マイグレーション」を含む

K. **`## テスト` セクション**が以下を含む：
- ユニット（`tests/unit/`）/ 結合（`tests/integration/`）/ E2E（`tests/e2e/`）の 3 階層
- Service の単体テストは Repository を `AsyncMock` でスタブ化する（Repository を明示的なインタフェース境界として置く設計のため、モックでも false positive を生まない、→ [ADR 0044](../../../adr/0044-backend-repository-pattern-adoption.md)）
- Repository の結合テストは実 DB（Testcontainers / docker-compose）に対して SQL 挙動のみを検証
...
> **このプロジェクトでの具体例**：[01-roadmap.md](../01-roadmap.md) の 本フェーズに該当する行が、状態列 `✅ 完了` + 詳細手順列が本ファイルへのリンク `[r0-setup/backend-layers.md](./r0-setup/backend-layers.md)` になっている状態。古い表現（`🔴 未着手` / 未着手プレースホルダ / 旧リンク等）が残っていれば最終状態に合わせる。
...
- 前フェーズ：[backend.md](./backend.md)（apps/api workspace + Postgres + Alembic + 最小 health_check の配置元）
- 次フェーズ：[frontend-layers.md](./frontend-layers.md)
