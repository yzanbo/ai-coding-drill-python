...
- **trivial 例外**：`health_check` のような INSERT 1 行 / SELECT 1 行レベルは Service を作らず router 直書きを許容（[backend.md: 7. 薄い CRUD エンドポイント](./backend.md#7-薄い-crud-エンドポイント--pytest-疎通) の設計判断に従う）
