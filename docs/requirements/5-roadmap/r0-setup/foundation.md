...
言語非依存の最低限の開発慣行を全 ✅ 完了済として記録する。本フェーズが終わると以下が揃う：

- commit 規約が機械強制される（commitlint）
- Git フックで commit 時の自動チェックが走る（lefthook）
- 言語ランタイム + ツールを `mise install` 1 発で再現できる
- GitHub Actions の CI 雛形が動き、`ci-success` umbrella ジョブで Branch protection に 1 行登録するだけで済む
- Dependabot の雛形が配置済で、エコシステム追加時にコメントアウト解除だけで自動更新 PR が走り始める

以降の役割別フェーズ（Backend / Frontend / Worker）はこの土台の上に「環境構築 + 品質ゲート 5 ステップ」パターンを積み上げる。
