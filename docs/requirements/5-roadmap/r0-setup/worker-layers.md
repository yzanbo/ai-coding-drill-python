...
- [01-roadmap.md](../01-roadmap.md) の R0-7 行が、状態列 `✅ 完了` + 詳細手順列が本ファイルへのリンク `[r0-setup/worker-layers.md](./r0-setup/worker-layers.md)` になっている（本フェーズ完了前の `🔴 未着手` 表記 + `（要作成）` 注記は完了時に書き換える）
