...
> **前提フェーズ**：[Python 環境構築フェーズ](./02-backend.md) 完了済（`apps/api/app/main.py` / `core/config.py` / `db/session.py` / `db/base.py` / `models/health_check.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py` が配置済、Postgres + Alembic の最小マイグレーションが通り、`mise run api:dev` で起動可能、ruff / pyright / pip-audit / deptry がローカル + CI 両方で緑）
...
5. **初期状態のばらつきに対する方針**：
   ...
   - 想定フォルダが存在し中身がある（既存の `routers/` `schemas/` `models/` `core/` `db/` 等、02-backend.md フェーズで埋まったもの） → 中身を確認して `§決定値` と矛盾する部分のみ書き換える。`02-backend.md` 由来の実装ファイル（`config.py` / `session.py` / `base.py` / `health_check.py` / `health.py`（schemas）/ `health.py`（router）/ `probes.py`）は本フェーズでは触らない（02 の SSoT を尊重する）
...
```text
apps/api/app/
├── __init__.py                                      # 02-backend.md 配置済
├── main.py                                          # FastAPI アプリ生成 + ルータ束ねる（02-backend.md 配置済）
├── README.md                                        # 全体図 + やってはいけないこと（本フェーズで作成）
├── core/                                            # 設定 / セキュリティ / 例外ハンドラ等の横断ユーティリティ（終端）
│   ├── __init__.py                                  # 02-backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   └── config.py                                    # pydantic-settings ベースの Settings + get_settings()（02-backend.md 配置済）
├── db/                                              # SQLAlchemy エンジン / セッション / Base
│   ├── __init__.py                                  # 02-backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   ├── base.py                                      # DeclarativeBase（02-backend.md 配置済）
│   └── session.py                                   # AsyncEngine + AsyncSessionLocal + get_async_session（02-backend.md 配置済）
├── models/                                          # SQLAlchemy モデル（1 テーブル 1 ファイル）
│   ├── __init__.py                                  # 全モデルを re-export（Alembic autogenerate が拾う）（02-backend.md 配置済）
│   ├── README.md                                    # 本フェーズで作成
│   └── health_check.py                              # 疎通確認用最小テーブル（02-backend.md 配置済）
├── schemas/                                         # Pydantic モデル（SSoT、HTTP API + Job キュー両境界に展開、ADR 0006）
│   ├── __init__.py                                  # 02-backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   └── health.py                                    # HealthCheckResponse（02-backend.md 配置済）
├── routers/                                         # APIRouter（機能別 controller 相当、薄い受付係）
│   ├── __init__.py                                  # 02-backend.md 配置済
│   ├── README.md                                    # 本フェーズで作成
│   ├── health.py                                    # POST/GET /health（DB 往復、02-backend.md 配置済）
│   └── probes.py                                    # GET /healthz（liveness、02-backend.md 配置済）
...
> **既存実装ファイル**（02-backend.md 配置済、本フェーズでは触らない）：`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `models/__init__.py`（`from app.models.health_check import HealthCheck` を re-export）/ `schemas/health.py` / `routers/health.py` / `routers/probes.py`。中身の SSoT は [02-backend.md](./02-backend.md) の各 step。
>
...
- **trivial 例外**：`health_check` のような INSERT 1 行 / SELECT 1 行レベルは Service を作らず router 直書きを許容（[02-backend.md: 7. 薄い CRUD エンドポイント](./02-backend.md#7-薄い-crud-エンドポイント--pytest-疎通) の設計判断に従う）
...
- `apps/api/app/` 配下が §A のツリーに一致している：
  ...
  - `main.py` が存在し、`include_router(probes.router)` / `include_router(health.router)` で 02-backend.md 配置済の 2 router を登録している
...
**02-backend.md 完了済を前提とした、本フェーズで実際に新規追加する差分**：

> 前提フェーズ（[02-backend.md](./02-backend.md)）完了時点で、`routers/` / `schemas/` / `models/` / `core/` / `db/` の 5 ディレクトリと、それらに属する実装ファイル（`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/__init__.py` + `models/health_check.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`）は既に配置済。**本フェーズはその上に「足りないレイヤ 3 つ」と「全レイヤの README + 実装契約」を載せるだけ**で、02-backend.md 由来の既存ファイルには手を入れない。
>
> | 差分カテゴリ | 中身 |
> |---|---|
...
> | **触らないもの** | 02-backend.md 配置済の `main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `models/__init__.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`、および `alembic/` / `tests/` / `pyproject.toml` / `uv.lock` / `.env.example` |

**完了基準**：

- 上記 9 個の README.md が（空でも可）存在する
- 8 レイヤすべてが空ではない（`__init__.py` + README 入り）
- 02-backend.md 配置済の実装ファイル（`main.py` / `core/config.py` / `db/base.py` / `db/session.py` / `models/health_check.py` / `schemas/health.py` / `routers/health.py` / `routers/probes.py`）に変更が入っていない
...
| # | 項目 | 何のために（要点）| 根拠 |
|---|---|---|---|
| 1 | アプリ設定値の管理（`config.py`）| `.env` / 環境変数から読み込んだ設定値を型付きで一元管理（pydantic-settings ベースの `Settings` + `get_settings()`）| 02-backend.md step 4 配置済 |
...
| # | 項目 | 何のために（要点）| 根拠 |
|---|---|---|---|
| 1 | DB セッションの貸し出し（`get_async_session`）| リクエスト開始時に開き、終了時に確実に閉じる処理を 1 箇所に集約。閉じ忘れによる接続プール枯渇を構造的に防ぐ | 02-backend.md step 4 で配置済 / R1 で `deps/db.py` に移植 |
...
| 8 | ヘルスチェック（`/healthz` / `/readyz`）| k8s / ALB がインスタンスの受付可否を機械可読に判定、デプロイ時の無停止切替を支える | 02-backend.md step 8 で `/healthz` 配置済、`/readyz` は R1 以降 |
...
- 前フェーズ：[02-backend.md](./02-backend.md)（apps/api workspace + Postgres + Alembic + 最小 health_check の配置元）
