...
### Now：R0 基盤（直列、初期慣行 + 言語別環境構築 + レイヤ分割 + MCP 整備）
...
> **前提**：R0 全完了（Go 環境構築 R0-4、Backend レイヤ分割 R0-5、Frontend レイヤ分割 R0-6、Worker レイヤ分割 R0-7 を含む。MCP R0-8 は開発体験向上で R1 のブロッカーではない）。R1-3 LLM プロバイダ抽象化が Worker コードを必要とするため、R0-4 / R0-7 が遅延した場合は R1-3 着手前に必ず完了させる。R0-5 / R0-6 / R0-7 が未完了の場合は R1-1 以降の機能実装着手前に必ず完了させる（`.claude/rules/{backend,frontend,worker}.md` の実装契約に沿って実装するため）。
