...
| R0-4 | Worker（Go）環境構築（apps/workers/grading + 品質ゲート + CI + dependabot 統合 + サンドボックス雛形）<br>**着手タイミングは柔軟**：R0-1〜R0-3 完了後にすぐ着手する必要はなく、**R1-2（LLM プロバイダ抽象化）着手前までに完了**させればよい（R1-2 が初めて Worker コードを書く項目で、R1-1 OAuth では Worker に触らないため） | 🔴 未着手 | [r0-setup/worker.md](./r0-setup/worker.md) |
| R0-5 | Backend レイヤ分割（`apps/api/app/{routers,services,schemas,models,core,db,deps,observability}/` を `__init__.py` 付きで配置、各 layer の責務 + import 方向 + 命名規則を `.claude/rules/backend.md` に「実装契約」として確定）| ✅ 完了 | [r0-setup/backend-layers.md](./r0-setup/backend-layers.md) |
| R0-6 | Frontend レイヤ分割（`apps/web/src/{components,hooks,lib,__generated__/api}/` 配置、feature folder 配置規約 + import 方向 + 命名規則を `.claude/rules/frontend.md` に「実装契約」として確定）| ✅ 完了 | [r0-setup/frontend-layers.md](./r0-setup/frontend-layers.md) |
| R0-7 | Worker（Go）ディレクトリ構成（`apps/workers/grading/{cmd,internal/{jobtypes,llm,sandbox,grading,db,observability}}/` 配置、各 package の責務 + import 方向 + 命名規則を `.claude/rules/worker.md` に「実装契約」として確定）<br>**前提**：R0-4（Go 環境構築）完了後に着手。R1-2 LLM プロバイダ抽象化の前に揃っている必要あり | 🔴 未着手 | [r0-setup/worker-layers.md](./r0-setup/worker-layers.md)（要作成） |
...
| R0-9 | 型同期パイプライン基盤構築（FastAPI → OpenAPI 3.1 export スクリプト + Pydantic → JSON Schema export スクリプト + Hey API による TS / Zod / 型付き HTTP クライアント生成 + CI 必須ステップ組込み。既存の `HealthCheckResponse` のみで end-to-end 疎通を確認できる粒度。**具体的なジョブペイロード Pydantic 定義は機能側に分散**：R1-1 OAuth / R1-2 LLM provider / R1-3 F-02 / R1-5 F-04 等の実装時に各機能と一緒に Pydantic モデルを追加し、`mise run types-gen` で各言語に展開する）<br>**前提**：R0-5（Backend レイヤ分割）完了 | 🔴 未着手 | [r0-setup/type-sync-pipeline.md](./r0-setup/type-sync-pipeline.md)（要作成）/ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md) |

**マイルストーン**：「Backend が完成 = R0-2 完了」「Frontend が完成 = R0-3 完了」「Worker が完成 = R0-4 完了」「Backend レイヤ確定 = R0-5 完了」「Frontend レイヤ確定 = R0-6 完了」「Worker レイヤ確定 = R0-7 完了」「MCP 整備 = R0-8 完了」「型同期パイプライン構築 = R0-9 完了」「R0 全完了 = `docker compose up && mise run api:dev && mise run web:dev && mise run worker:grading:dev && mise run types-gen` で開発環境が全言語で立ち上がり、型同期パイプラインも end-to-end で通り、Backend / Frontend / Worker のレイヤ規約が rules ファイルに固定 + MCP サーバーが Claude Code から使える状態」。
...
> **前提**：R0 のうち **R1 開始時点で必須**なのは **R0-5（Backend レイヤ分割）/ R0-6（Frontend レイヤ分割）/ R0-9（型同期パイプライン基盤）** の 3 つ（R1-1 OAuth で Backend / Frontend コードを書き始め、機能ごとに Pydantic を追加して `mise run types-gen` で展開するため、`.claude/rules/{backend,frontend}.md` の実装契約と型同期パイプラインが確定している必要がある）。**R0-4（Worker Go 環境構築）/ R0-7（Worker レイヤ分割）は R1-2 着手前まで** に完了させればよい（R1-2 LLM プロバイダ抽象化が初めて Worker コードを書く項目で、R1-1 では Worker に触らないため、R0-4 / R0-7 は R1-1 と並行で進めて差し支えない）。**R0-8（MCP）は R1 のブロッカーではない**（開発体験向上）。
...
| ⏸️ | **R0-4 + R0-7 完了**（R1-2 ブロッカー解消フェーズ：Worker Go 環境構築 + Worker レイヤ分割をここで消込。Just-in-time でツール版数を最新に保つため R1-1 完了後に実行） | 🔴 | [R0-4](./r0-setup/worker.md) / [R0-7](./r0-setup/worker-layers.md)（要作成） |
...
| Pydantic SSoT → OpenAPI / JSON Schema → TS / Go 型生成の同期が崩れる | Frontend / Worker が古い型で動き、型安全性が形骸化 | R0-9 で `mise run api:openapi-export` / `api:job-schemas-export` / `web:types-gen` を CI 必須ステップに組み込み、生成物の差分が出たら CI を fail させる。`worker:types-gen` は R0-4 / R0-7 完了後（R1-2 着手時）に CI 必須ステップへ追加する → [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md) / [ADR 0039](../../adr/0039-mise-for-task-runner-and-tool-versions.md) |
