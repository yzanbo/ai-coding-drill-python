...
任意（優先度低、Later 扱い）：R6 適応型学習 / R7 問題生成 Worker（apps/workers/generation）/ R8 多言語化 / R9 Firecracker microVM。
...
| R0-3 | mise によるタスクランナー兼 tool 版数 SSoT 確立（Python / pnpm / Go の版数固定 + 横断タスク定義）| 🔴 | Turborepo 不採用後の横断タスク基盤 → [ADR 0039](../../adr/0039-mise-for-task-runner-and-tool-versions.md) |
| R0-4 | uv による Python workspace 初期化（apps/api 雛形 + lockfile + ruff / pyright）| 🔴 | Python pivot の最初の足場 → [ADR 0035](../../adr/0035-uv-for-python-package-management.md) / [ADR 0020](../../adr/0020-python-code-quality.md) |
| R0-5 | apps/web 内に閉じた pnpm + Biome + Knip + tsconfig 構成 | 🔴 | root に Frontend ツーリングを置かない → [ADR 0036](../../adr/0036-frontend-monorepo-pnpm-only.md) / [ADR 0018](../../adr/0018-biome-for-tooling.md) |
| R0-6 | GitHub Actions（commitlint + ruff/pyright + Biome + go vet/lint） | 🔴 | リモートで規約逸脱を弾く → [ADR 0026](../../adr/0026-github-actions-incremental-scope.md) / [ADR 0025](../../adr/0025-github-actions-as-ci-cd.md) |
| R0-7 | Dependabot + GitHub Actions の SHA ピン止め | 🔴 | 設定 1 ファイルで自動 PR、SHA ピン止めとセット運用 |
| R0-8 | Docker Compose（Postgres + Redis）+ SQLAlchemy 2.0（async） + Alembic 初版 + マイグレーション基盤 | 🔴 | R0 完了の最終条件 → [ADR 0004](../../adr/0004-postgres-as-job-queue.md) / [ADR 0037](../../adr/0037-sqlalchemy-alembic-for-database.md) |

R0 で着手しないツール（後続フェーズ待ち）：

- `gofmt` + `golangci-lint` — R1（apps/workers/grading 着手時）
- Pydantic から JSON Schema 出力（Worker 向け） + FastAPI OpenAPI 3.1（Frontend 向け型生成） — R1（最初のスキーマ投入時。雛形だけ R0 で用意可。→ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md)）
- pytest / Vitest / Playwright — テスト対象コード待ち（→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- ミューテーションテスト — R2 以降
...
| ID | 項目 | 状態 | 関連 |
|---|---|---|---|
| R1-1 | ジョブペイロード Pydantic モデル確定 + Pydantic から JSON Schema 出力（Go 型生成パス）+ FastAPI OpenAPI 3.1（Frontend 向け TS 型生成）の構築（[補足](#r1-1-補足-型同期パイプライン)）| 🔴 | [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md) |
...
| R1-6 | apps/workers/grading（Go）初版 + サンドボックス（Docker + 制限フラグ）+ Go リンター導入（[補足](#r1-6-補足-go-リンター導入)）| 🔴 | [ADR 0016](../../adr/0016-go-for-grading-worker.md) / [ADR 0009](../../adr/0009-disposable-sandbox-container.md) / [ADR 0019](../../adr/0019-go-code-quality.md) / [ADR 0040](../../adr/0040-worker-grouping-and-llm-in-worker.md) |
...
- **Frontend 向け（TS 型）**：FastAPI が自動生成する `apps/api/openapi.json`（OpenAPI 3.1）を起点に、`apps/web` 内で `openapi-typescript` 等で TS 型を生成。`mise run gen:web-types` 等のタスクとして mise.toml に登録（→ [ADR 0039](../../adr/0039-mise-for-task-runner-and-tool-versions.md)）
- **Worker 向け（Go 型）**：Pydantic モデルから `model.model_json_schema()` で JSON Schema を出力し、`apps/workers/grading/internal/schema/` に Go 型として生成。`mise run gen:worker-types` 等のタスクとして登録
...
- 中継パッケージ（旧 `packages/shared-types`）は廃止（→ [ADR 0036](../../adr/0036-frontend-monorepo-pnpm-only.md) 拡張）。各アプリは生成物を自身のディレクトリ内に配置する
...
採点ワーカーで Go コードがリポジトリに初めて入る時点で、Python / TS と同等の品質ゲートを揃える。**R1-6 の DoD に含める**（→ [ADR 0019](../../adr/0019-go-code-quality.md) / [ADR 0040](../../adr/0040-worker-grouping-and-llm-in-worker.md)）。

- `apps/workers/grading/` 配下に **`go.mod` + `.golangci.yml`** を新設
...
- CI（R0-6 で整備した GitHub Actions）にも `golangci-lint run` ステップを追加（`mise run lint:worker` 経由 → [ADR 0039](../../adr/0039-mise-for-task-runner-and-tool-versions.md)）
...
| 🟢 | apps/web 内の pnpm 依存整合性の正規化（`strict-peer-dependencies=true` 復帰 + `pnpm.overrides` 整理） | R5 | [apps/web/.npmrc](../../../apps/web/.npmrc) / [ADR 0036](../../adr/0036-frontend-monorepo-pnpm-only.md) |
...
| SQLAlchemy 2.0 async + Alembic の運用未経験箇所での詰まり | DB 層の進捗が止まると R1 全体が遅延 | R0-8 で薄い CRUD + マイグレーション 1 周を通し、つまずき箇所を早期に洗い出す → [ADR 0037](../../adr/0037-sqlalchemy-alembic-for-database.md) |
...
| 立ち上げ期の apps/web 内 pnpm peer 緩和設定を本番化前に戻し忘れる | 依存バージョン不整合が実行時エラーや謎の挙動として顕在化、デバッグ困難化 | R5 のバックログ「apps/web 内 pnpm 依存整合性の正規化」で `strict-peer-dependencies=true` 復帰を必須化 → [apps/web/.npmrc](../../../apps/web/.npmrc) / [ADR 0036](../../adr/0036-frontend-monorepo-pnpm-only.md) |
| Pydantic SSoT → OpenAPI / JSON Schema → TS / Go 型生成の同期が崩れる | Frontend / Worker が古い型で動き、型安全性が形骸化 | R1-1 で `mise run gen:*` を CI 必須ステップに組み込み、生成物の差分が出たら CI を fail させる → [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md) / [ADR 0039](../../adr/0039-mise-for-task-runner-and-tool-versions.md) |
