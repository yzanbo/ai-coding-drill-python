...
| ⏸️ | **R0-8 + R0-9 + R0-11 完了**（R1-2 ブロッカー解消フェーズ：Worker Go 環境構築 + Worker レイヤ分割 + Worker 側型同期パイプライン合流をここで消込。Just-in-time でツール版数を最新に保つため R1-1 完了後に実行） | 🔴 | [R0-8](./r0-setup/worker.md) / [R0-9](./r0-setup/worker-layers.md)（要作成）/ R0-11（要作成） |
