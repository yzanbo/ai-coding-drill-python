...
| R0-4 | Go 環境構築（apps/workers/grading + 品質ゲート + CI + dependabot 統合 + サンドボックス雛形）<br>**着手タイミングは柔軟**：R0-1〜R0-3 完了後にすぐ着手する必要はなく、**R1 開始までに完了**させればよい（R1-5 LLM プロバイダ抽象化が Worker コードを必要とするため、その前までに揃っていればよい） | 🔴 未着手 | [r0-setup/04-go.md](./r0-setup/04-go.md) |
...
- Pydantic → JSON Schema / OpenAPI 3.1 の型同期パイプライン → R1-3（雛形のみ R0 で用意可、→ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md)）
...
> **前提**：R0 全完了（Go 環境構築 R0-4、Backend レイヤ分割 R0-5、Frontend レイヤ分割 R0-6 を含む）。R1-5 LLM プロバイダ抽象化が Worker コードを必要とするため、R0-4 が遅延した場合は R1-5 着手前に必ず完了させる。R0-5 / R0-6 が未完了の場合は R1-3 以降の機能実装着手前に必ず完了させる（`.claude/rules/{backend,frontend}.md` の実装契約に沿って実装するため）。
