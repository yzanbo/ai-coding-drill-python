...
| R0-2 | Python 環境構築（apps/api + DB + 品質ゲート + CI + dependabot 統合） | 🔴 未着手 | [r0-setup/02-python.md](./r0-setup/02-python.md) |
| R0-3 | Next.js 環境構築（apps/web + 品質ゲート + CI + dependabot 統合） | 🔴 未着手 | [r0-setup/03-nextjs.md](./r0-setup/03-nextjs.md) |
...
### Now：R1 MVP（7 項目、上から順に依存関係に従う）

> **前提**：R0 全完了（Go 環境構築 R0-4 を含む）。R1-3 LLM プロバイダ抽象化が Worker コードを必要とするため、R0-4 が遅延した場合は R1-3 着手前に必ず完了させる。
