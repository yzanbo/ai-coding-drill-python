...
### Now：R0 基盤（直列、初期慣行 + 言語別環境構築 + レイヤ分割）
...
> **言語 setup とレイヤ分割の分担**：r0-setup 配下の言語別ファイル（[02-python.md](./r0-setup/02-python.md) / [03-nextjs.md](./r0-setup/03-nextjs.md) / [04-go.md](./r0-setup/04-go.md)）では「悩む余地のない基本構造」（`app/main.py` / `app/db/session.py` / Next.js scaffold 等）までを扱う。**R0-5 / R0-6** はその上に **プロジェクト固有のレイヤ分割**（FastAPI の `routers` / `services` / `schemas` / `core` 分離 + import 方向、Frontend の feature folder vs `src/{components,hooks,lib}` 配置規約）を確定し、`.claude/rules/{backend,frontend}.md` に「実装契約」として固定する。R1 以降の機能実装はこの規約に従う。
...
| R0-6 | Frontend レイヤ分割（`apps/web/src/{components,hooks,lib,__generated__/api}/` 配置、feature folder 配置規約 + import 方向 + 命名規則を `.claude/rules/frontend.md` に「実装契約」として確定）| 🔴 未着手 | （着手時に r0-setup 配下に詳細手順ファイルを作成）|

**マイルストーン**：「Python が完成 = R0-2 完了」「Next.js が完成 = R0-3 完了」「Go が完成 = R0-4 完了」「Backend レイヤ確定 = R0-5 完了」「Frontend レイヤ確定 = R0-6 完了」「R0 全完了 = `docker compose up && mise run api:dev && mise run web:dev && mise run worker:grading:dev` で開発環境が全言語で立ち上がり、Backend / Frontend のレイヤ規約が rules ファイルに固定されている」。
...
> **前提**：R0 全完了（Go 環境構築 R0-4、Backend レイヤ分割 R0-5、Frontend レイヤ分割 R0-6 を含む）。R1-3 LLM プロバイダ抽象化が Worker コードを必要とするため、R0-4 が遅延した場合は R1-3 着手前に必ず完了させる。R0-5 / R0-6 が未完了の場合は R1-1 以降の機能実装着手前に必ず完了させる（`.claude/rules/{backend,frontend}.md` の実装契約に沿って実装するため）。
