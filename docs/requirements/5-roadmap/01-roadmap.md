...
| R0-7 | 型同期パイプライン基盤構築（FastAPI → OpenAPI 3.1 export スクリプト + Pydantic → JSON Schema export スクリプト + Hey API による TS / Zod / 型付き HTTP クライアント生成 + CI 必須ステップ組込み。既存の `HealthCheckResponse` のみで end-to-end 疎通を確認できる粒度。**具体的なジョブペイロード Pydantic 定義は機能側に分散**：R1-1 OAuth / R1-2 LLM provider / R1-3 F-02 / R1-5 F-04 等の実装時に各機能と一緒に Pydantic モデルを追加し、`mise run types-gen` で各言語に展開する）<br>**前提**：R0-4（Backend レイヤ分割）完了 | 🔴 未着手 | [r0-setup/type-sync-pipeline.md](./r0-setup/type-sync-pipeline.md)（要作成）/ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md) |
...
| ID | 項目 | 状態 | 関連 |
|---|---|---|---|
| R1-1 | [F-01: GitHub OAuth ログイン](../4-features/F-01-github-oauth-auth.md) | 🔴 | [ADR 0011](../../adr/0011-github-oauth-with-extensible-design.md) |
...
| R1-3 | [F-02: 問題生成リクエスト](../4-features/F-02-problem-generation.md)（API は enqueue のみ、生成は grading worker でも問題生成 worker でも実行可能な形に）| 🔴 | [03-llm-pipeline.md](../2-foundation/03-llm-pipeline.md) / [ADR 0040](../../adr/0040-worker-grouping-and-llm-in-worker.md) |
| R1-4 | [F-03: 問題表示・解答入力](../4-features/F-03-problem-display-and-answer.md)（**着手時に TanStack Query を導入** — Provider 設置 + Hey API `@tanstack/react-query` プラグイン有効化、→ [ADR 0042](../../adr/0042-frontend-data-fetching-tanstack-query.md)）| 🔴 | [ADR 0015](../../adr/0015-codemirror-over-monaco.md) / [ADR 0042](../../adr/0042-frontend-data-fetching-tanstack-query.md) |
| R1-5 | [F-04: 自動採点](../4-features/F-04-auto-grading.md)（trace_id 連結を R1 から実装）| 🔴 | [ADR 0010](../../adr/0010-w3c-trace-context-in-job-payload.md) |
| R1-6 | [F-05: 学習履歴・統計](../4-features/F-05-learning-history.md) | 🔴 | — |
