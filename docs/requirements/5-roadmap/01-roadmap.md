...
| R0-3 | モノレポ枠（pnpm workspaces + Turborepo + `packages/config`）| ✅ | 後続ツールの設定置き場 → [ADR 0012](../../adr/0012-turborepo-pnpm-monorepo.md) |
...
旧「Phase X」表記との対応は [ADR 0019](../../adr/0019-requirements-as-5-buckets.md) 参照（Phase 0〜9 = R0〜R9）。ADR 本文に残る「Phase X」は Append-only 原則のため変更しない（リリース指示子として読み替える）。
