...
- **キャンセル機能は提供しない**（MVP 時点）：Worker 着手前のキャンセルだけ可能でも UX 上は「いつ押せるか分からないボタン」になり、Worker 着手後の本物のキャンセルには checkpoint 機構（issue #89 / #90、R7 で Worker 本実装と一緒に検討）が必要なため、半端な擬似キャンセルは廃止した。`status` カラムは `pending` / `completed` / `failed` の 3 値で運用する（既存 DB に残る `canceled` 行は表示互換のため許容、新規には作らない）
...
- **生成所要時間と失敗理由の永続化**：`completed` / `failed` 遷移時に `completed_at` を書き、`failed` の時は `failure_reason` も書く。履歴画面で「経過時間 / 所要時間」「失敗理由」を表示するために必要
...
| 生成再試行 | 認証ユーザー | 必須 | `POST /api/me/generations/:id/retry` で `failed` のリクエストを新規 generation_request として複製。生成中ページ / 生成履歴ページの両方で同じボタンから呼べる | [#履歴上のアクション](#履歴上のアクション) |
...
- **主要インタラクション**：
  ...
  - `status === 'failed'`（最大 3 回再生成しても全失敗）で再試行ボタンを表示。押すと同じカテゴリ・難易度で新規 generation_request を作り、新しい `/problems/generate/:newRequestId` に置換遷移する（生成履歴ページの再試行と同じ hook を共有：`apps/web/src/hooks/use-retry-my-generation/`）
  - `status === 'failed'` の補助導線として「条件を変えてやり直す」テキストリンクを再試行ボタン直下に置く（`/problems/new` へ）。別カテゴリ・難易度で作り直したい時 / 他タブで既に retry 済みで 409 が返った時の救済経路
...
- **主要インタラクション**：
  - **行クリック**：状態に応じて遷移先を分岐
    ...
    - `failed` / `canceled` → 遷移なし。行内に失敗理由 / キャンセル済を表示（`canceled` は旧仕様で書き込まれた既存データの表示互換のみ、新規には作らない）
  - **再試行**：`failed` 行のみボタン表示。押すと新規 generation_request が作成され、ヘッダーに新しい行が追加される。生成中ページの再試行ボタンと同じ hook（`apps/web/src/hooks/use-retry-my-generation/`）を共有する
- **ポーリング**：
  - **1 秒間隔**で `GET /api/me/generations` を再フェッチ
  - 全件が終端状態（`completed` / `failed`、および旧データの `canceled`）になったら停止（無駄な API 呼び出しを抑える）
...
- **再試行**：`failed` のみ対象。生成中ページと生成履歴ページのどちらの再試行ボタンも同じ Backend エンドポイント（`POST /api/me/generations/:id/retry`）+ 同じ FE hook（`apps/web/src/hooks/use-retry-my-generation/`）を呼ぶ。新規 `generation_request` を作成（同じ category / difficulty）+ `retry_of=<直前の親 ID>` でリンク（チェーンの根 ID ではなく、ユーザーが押した failed 行 1 つだけを指す。N 回目の再試行は前回の retry レコードを親にする線形チェーンとして伸びる）+ 新規 `jobs` INSERT + NOTIFY。元レコードは status='failed' のまま残る（履歴としてトレース可能）。チェーン全体の N 回目はサーバ側で `WITH RECURSIVE` の深さ計算（`retryCount`）として返す
- **所有権チェック**：`Submission.user_id == current_user.id` 相当の WHERE を必ず付け、他人のリクエストを 404 で隠す（権限漏洩防止）
- **キャンセル機能の廃止**：MVP 時点ではキャンセル機構を提供しない（Worker 着手後の本物のキャンセルが必要、issue #89 / #90 で R7 検討）
...
- [ ] 全件が終端状態（completed / failed、旧データの canceled 含む）になるとポーリングが止まる
- [ ] タブ非アクティブの間はポーリングが止まる（タブを戻すと再開）
- [ ] `failed` 行（生成中ページの failed 表示 / 生成履歴ページの failed 行）に「再試行」ボタンが表示され、押すと新規 generation_request が作成され `retry_of` で元 ID が記録される。両画面で同じ Backend / FE hook を呼ぶ
...
- [ ] 他人の generation_request を `/api/me/generations/:id/retry` で操作しようとすると 404 が返る
...
- [ ] バックエンド実装完了（generation ルーター：enqueue + ステータス取得 + 履歴一覧 + 再試行、LLM 呼び出しは含めない、→ [ADR 0040](../../adr/0040-worker-grouping-and-llm-in-worker.md)）
...
- [ ] E2E テスト完了（生成 → 完了 → 問題遷移の主要フロー + 履歴画面の表示 + 生成中 / 履歴の両ページでの再試行、Playwright、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
