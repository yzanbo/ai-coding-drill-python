...
- **表示項目**：
  ...
  - **失敗理由はステータス画面と同様、内部種別を区別せず丸めた汎用文言を表示する**（情報漏洩防止、§94 の方針と同一）。DB の `failure_reason` カラムは運用ログ用途で内部種別タグを保持するが、UI には出さない
...
- [ ] `failed` 行に失敗理由が表示される（DB の `failure_reason` 生タグではなく、ステータス画面と同じく丸めた汎用文言を表示）
