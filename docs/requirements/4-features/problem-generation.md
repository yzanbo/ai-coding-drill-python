...
- **失敗理由タグ taxonomy**：`failure_reason` は Worker が以下の固定タグから 1 つを書き込む。ops は DB から `SELECT failure_reason, count(*) FROM generation_requests WHERE status='failed' GROUP BY 1` で頻度分布を取り、どこを改善すべきか（プロンプト改善 / LLM プロバイダ切替 / threshold 調整 / sandbox 安定化）の優先度判断に使う。**API レスポンスにはタグ自体を含めない**（情報漏洩防止、§生成履歴画面 §表示項目の方針と同一）。UI は `status==='failed'` だけ見て丸めた汎用文言を表示する：
  ...
  - `max_attempts_exceeded` — 上記いずれにも分類できない fallback（LLM 一過性エラー（429 / NW タイムアウト）累積 / Docker daemon ハング累積 等）
...
- **表示項目**：
  ...
  - **失敗理由はステータス画面と同様、内部種別を区別せず丸めた汎用文言を表示する**（情報漏洩防止、§94 の方針と同一）。DB の `failure_reason` カラムは運用ログ用途で内部種別タグを保持するが、**API レスポンスにフィールド自体を含めない**（FE は `status==='failed'` だけ見て固定文言を出す。DevTools / curl 経由での内部状態漏洩を API 境界で塞ぐ）
...
- **再試行**：`failed` のみ対象。新規 `generation_request` を作成（同じ category / difficulty）+ `retry_of=<元 ID>` でリンク + 新規 `jobs` INSERT + NOTIFY。元レコードは status='failed' のまま残る（履歴としてトレース可能）
...
- [ ] `failed` 行に失敗理由が表示される（API は `failure_reason` フィールドを返さず、FE は status==='failed' で固定の汎用文言を表示する）
