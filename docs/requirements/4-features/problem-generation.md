...
- **キャンセル可能状態**：`status='pending'`（Worker 未着手）のリクエストのみキャンセル可能。`running`（Worker 処理中）はキャンセルしない（LLM 呼び出し中の中断はコスト削減効果が小さく、Worker 側のキャンセル機構を持たない方が実装単純）
...
- **主要インタラクション**：
  - **行クリック**：状態に応じて遷移先を分岐
    ...
    - `pending` / `running` → `/problems/generate/:requestId`（既存のステータス画面）
...
```json
{ "detail": "generation request is not cancelable (status=running)" }
```
...
- [ ] 進行中（pending / running）の行がある間は 1 秒間隔でポーリングが走り、状態遷移が画面に反映される
...
- [ ] `failed` 行に失敗理由（failure_reason）が表示される
