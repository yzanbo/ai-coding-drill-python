...
- **主要画面はすべて認証必須**：`/problems` / `/problems/:id` / `/me/*` への未ログインアクセスは `/login?next=...` に server-side redirect される（R1-6 で /problems も認証必須化、→ [./problem-display-and-answer.md](./problem-display-and-answer.md) / [3-cross-cutting/03-page-routing.md](../3-cross-cutting/03-page-routing.md)）。未ログインで触れる画面は `/`（ランディング）と `/login` のみ
...
- [x] ログイン後、`GET /auth/me` が 200 で現在ユーザーの情報を返す
- [x] セッションは 7 日間有効で、ユーザーが操作するたびに有効期限が延長される
- [x] ログアウトボタン押下後、`GET /auth/me` が 401 を返し未認証状態に戻る
- [x] セッション期限切れ後にアクセスすると 401 が返り、再ログインを要求される
- [x] 認証必須エンドポイントに未認証でアクセスすると 401 が返る（デフォルト認証必須）

**§2 GitHub プロバイダ実装に紐づく項目**

- [x] ヘッダーまたは `/login` 画面に「GitHub でログイン」ボタンが表示される
- [x] ボタン押下で GitHub の認可画面に遷移する
- [x] 認可後、自動的にコールバック処理が走り、ログイン状態でホーム画面に遷移する
- [x] 同じ GitHub アカウントで再ログインしても、`GET /auth/me` が返すユーザー ID が初回と同一（重複ユーザーが作られない）
- [x] `state` 不一致のコールバックでは新規セッションが作られず `/login?auth_error=state_invalid` へ 302（CSRF 対策、Frontend がトースト表示）
- [x] `state` トークン発行から 10 分超のコールバックも同様に `/login?auth_error=state_invalid` へ 302（TTL 切れも CSRF と同じ扱い）
- [x] 同じ `state` でコールバックを 2 回送ると 2 回目も `/login?auth_error=state_invalid` へ 302（1 回使い切り）
- [x] GitHub プロフィールに `name` が設定済みのユーザーは `GET /auth/me` の `displayName` に `name` が返る、未設定なら `login` が返る
- [x] ログイン済みユーザーが `/login` を開くとホーム `/` へリダイレクトされる（`?next=` 指定があればそちら優先）
- [x] `/login?next=https://evil.com` のような外部 URL を指定してログインしてもホーム `/` に遷移する（外部誘導されない）
- [x] `/login?next=/%2F%2Fevil.com` のような URL エンコード経由の `//evil.com` を指定してもホーム `/` に遷移する（デコード後の検証で弾く）
- [x] GitHub 認可画面で Cancel を押すと `/login` に戻り、トーストで「ログインをキャンセルしました」等の通知が出る（セッションは作られない）
- [x] ログアウト成功後はホーム `/` に遷移する（`/login` には自動で飛ばない）
- [x] 同一ユーザーで PC とスマホからそれぞれログインした時、両方のセッションが同時に有効（先にログインした側が切れない）
