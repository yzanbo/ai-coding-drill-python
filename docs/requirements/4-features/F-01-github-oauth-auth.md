...
3. FastAPI が GitHub の認可 URL を生成してリダイレクト
4. ユーザーが GitHub で認可
5. GitHub が `GET /auth/github/callback?code=...` にリダイレクト
6. FastAPI が code を access_token に交換し、GitHub API でユーザー情報取得（`httpx.AsyncClient` 等の async クライアントを利用）
7. `auth_providers.provider_id` で既存ユーザー検索 → 既存なら再利用、新規なら `users` + `auth_providers` を同一トランザクションで作成（SQLAlchemy 2.0 async セッション）
...
機械可読の最新仕様は OpenAPI（`apps/api/openapi.json`、ランタイムは FastAPI の `/openapi.json`）が SSoT。本セクションは設計意図の記録。
...
- **横断要件**：
  - 認証アーキテクチャ：[2-foundation/02-architecture.md](../2-foundation/02-architecture.md#backend-apifastapi)
