...
- `jobs`：問題生成ジョブのキューイング（`type='generate-problem'`, `payload`, `state`）。詳細は [ADR 0004](../../adr/0004-postgres-as-job-queue.md)
...
5. **生成 Worker（`apps/workers/generation`）** が NOTIFY を受信、ジョブ取得
...
- [ ] 生成 Worker 実装完了（`apps/workers/generation`：LLM 呼び出し + サンドボックス検証 + Judge）
