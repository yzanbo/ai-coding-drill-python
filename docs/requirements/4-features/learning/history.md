...
- `submissions`：解答送信ごとに作成（[自動採点](../grading/auto-grading.md) で詳細）。`user_id`, `problem_id`, `status`, `score`, `created_at`, `graded_at` を集計に使用
- `problems`：カテゴリ・難易度を取得する目的で JOIN 対象

集計クエリイメージ（SQLAlchemy 2.0 で発行する SQL の等価）：
...
サーバ側で必ず `Submission.user_id == current_user.id` を WHERE に含める（SQLAlchemy 2.0、→ [backend.md: where 条件の必須ルール](../../../../.claude/rules/backend.md)、[ADR 0037](../../../adr/0037-sqlalchemy-alembic-for-database.md)）。

## 関連

- **関連機能**：
  - [自動採点](../grading/auto-grading.md)（履歴の元データを生成）
...
- [ ] 要件定義完了
- [ ] バックエンド実装完了（FastAPI me/stats / me/weakness ルーター、または submissions の拡張）
- [ ] フロントエンド実装完了（履歴一覧 / 統計 / 弱点画面、Hey API 生成クライアント）
