...
- [ ] 採点はサンドボックス環境で実行され、ホストやネットワークに影響を与えない（→ [ADR 0008](../../adr/0008-disposable-sandbox-container.md)）
- [ ] サンドボックスは使い捨て（1 ジョブ = 1 コンテナ）
- [ ] 採点ジョブの trace_id がリクエストから採点結果取得まで連結される（→ [ADR 0017](../../adr/0017-w3c-trace-context-in-job-payload.md)）
...
- **セキュリティ最優先**：ユーザーが書いたコードは攻撃コードである可能性を前提に扱う
  ...
  - 非 root 実行
  - 実行時間 5 秒上限
- **使い捨てコンテナ**：前回実行の影響が原理的に残らない（→ [ADR 0008](../../adr/0008-disposable-sandbox-container.md)）
...
- **trace_id の連結**：NestJS リクエスト → ジョブ → ワーカー処理 → 採点結果が単一トレースで追える（→ [ADR 0017](../../adr/0017-w3c-trace-context-in-job-payload.md)）
...
- gVisor / Firecracker への切り替え（R3 / 9）
...
- `jobs`：採点ジョブのキューイング（`type='grade'`, `payload: { traceContext, submissionId, problemId, code, language, timeoutMs }`、→ [3-cross-cutting/01-data-model.md: ジョブペイロード](../3-cross-cutting/01-data-model.md#ジョブペイロード共通フィールドtracecontext)、[ADR 0017](../../adr/0017-w3c-trace-context-in-job-payload.md)）
...
[2] NestJS（GradingController → GradingService）
    BEGIN;
      ...
      INSERT INTO jobs (type, payload, state) VALUES ('grade', { traceContext, ... }, 'queued') RETURNING id;
...
[4] Go ワーカー（別 VM、ずっと LISTEN 中）
    ...
    → COMMIT（行ロック解放）
    → payload.traceContext から OTel Context を復元、SpanLink で親に連結

[5] Go ワーカー（サンドボックスランナー）
...
詳細は [2-foundation/02-architecture.md: 1 ジョブが流れる完全な経路](../2-foundation/02-architecture.md#1-ジョブが流れる完全な経路) を参照。
...
- **関連 ADR**：
  ...
  - [ADR 0005: Go で採点ワーカーを実装](../../adr/0005-go-for-grading-worker.md)
  - [ADR 0008: 使い捨てサンドボックスコンテナ](../../adr/0008-disposable-sandbox-container.md)
