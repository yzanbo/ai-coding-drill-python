...
- **型エラー**：型パズル系カテゴリで `tsc --noEmit` が失敗した場合に「型エラー」と tsc 出力が表示される（カテゴリが `type-puzzle` のときだけ採点前に走る、issue #79）
...
- [x] 「実行」ボタン押下で `POST /submissions` に解答が送信され、`202 Accepted` + `submissionId` が即返る
- [x] フロントは `GET /submissions/:id` を 1〜2 秒間隔でポーリングし、採点結果を取得する
- [x] 全テストケース通過 → 「正解」表示 + 通過数 / 全数（例：5/5）
- [x] 一部失敗 → 失敗したテストケース名・期待値・実際の出力・差分を表示
- [x] 構文エラー / 実行時例外 → スタックトレースを整形して表示
- [x] タイムアウト（5 秒超過） → 「タイムアウト」と表示
- [x] OOM / メモリ超過 → 「メモリ使用量超過」と表示
- [x] 型パズル系カテゴリは型エラーの有無が採点結果に反映される（`tsc --noEmit` を Vitest の前段で実行、型エラー時は `failureKind=type_error` で `graded` 確定、issue #79）
- [x] 他ユーザーの `submissions/:id` には 403 / 404 が返って閲覧不可
- [x] レート制限：同一ユーザーで `1 分 / 20 回` を超えると `429` を返す（→ [3-cross-cutting/02-api-conventions.md](../3-cross-cutting/02-api-conventions.md#レート制限)）
- [x] 採点結果は再取得しても同じ内容が返る（永続保存）
...
- [x] バックエンド実装完了（submissions ルーター：enqueue + 結果取得のみ。サンドボックス処理は含めない）
- [x] バックエンドユニットテスト完了（pytest、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- [x] フロントエンド実装完了（採点結果表示コンポーネント）
- [x] フロントエンドユニットテスト完了（Vitest、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- [x] ワーカー実装完了（採点 Worker `apps/workers/grading`：ジョブ取得・サンドボックス（Docker + 制限フラグ、R3 で gVisor 切替）起動・結果書き戻し）
- [x] ワーカーユニットテスト完了（Go testing + testify、SandboxRunner のモックテスト含む、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- [x] E2E テスト完了（解答送信 → 採点完了 → 結果表示の主要フロー、Playwright、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- [x] **受け入れ条件すべて満たす**
