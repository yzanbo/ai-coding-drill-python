...
- [x] `/me/history` で自分の解答履歴一覧（問題タイトル / 結果 / 所要時間 / 解答日時）が表示される
- [x] `/me/stats` で正答率・カテゴリ別習熟度（解答数・正解数）が表示される
- [x] `/me/weakness` で弱点カテゴリ（正答率の低いカテゴリ Top N）が表示される
- [x] 同一問題に複数回解答した場合、すべての履歴が一覧に残る（上書きされていない）
- [x] 他ユーザーの `submissions` / `me/stats` / `me/weakness` には 403 / 404 が返って閲覧不可
- [x] 統計はリアルタイム計算で取得時点の値が返る（事前集計のキャッシュ古さ問題が発生しない）
- [x] 履歴ゼロのユーザーが `/me/stats` / `/me/weakness` を叩いても 200 が返り、`total=0` / `byCategory=[]` / `weakCategories=[]` の空集計として表示される
...
- [x] E2E テスト完了（解答送信 → 履歴反映 → 統計画面表示の主要フロー、Playwright、→ [ADR 0038](../../adr/0038-test-frameworks.md)）
- [x] **受け入れ条件すべて満たす**
