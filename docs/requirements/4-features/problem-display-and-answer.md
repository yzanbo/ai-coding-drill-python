...
- **問題一覧（`/problems`）は認証必須**：解答 / 履歴 / 統計までを一体の動線とし、未ログインで触れる画面はランディング（`/`）とログイン（`/login`）のみに絞る。ランディングの CTA も「GitHub でログイン」1 つに絞り、`/problems` への直接導線（押しても `/login` に弾かれるだけ）はランディング上に置かない。未ログインで `/problems` を直接踏んだ場合は server-side で `/login?next=/problems` に redirect する（フィルタ / ページ番号も保持して戻る）
...
- [ ] **受け入れ条件すべて満たす**<!-- インライン型診断（@typescript/vfs 経由の補完）1 項目のみ未充足、Next バックログで対応。 -->
