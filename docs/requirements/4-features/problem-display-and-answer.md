...
- [x] ゲストでも問題一覧・問題詳細を閲覧できる（401 にならない）
- [ ] 一覧画面のヘッダー領域に「新規問題を生成」ボタンがあり、`/problems/new`（[問題生成リクエスト](./problem-generation.md)）に遷移できる（ゲストの場合はログイン経由）
...
- [x] ゲストが「実行」を押すと `/login` にリダイレクトされる
