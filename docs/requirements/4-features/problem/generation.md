...
5. **生成 Worker** が NOTIFY を受信、ジョブ取得（R1〜R6 は `apps/workers/grading` が兼務、R7 で `apps/workers/generation` に切り出し → [01-roadmap.md](../../5-roadmap/01-roadmap.md) R1-3 / R7）
...
- **関連機能**：
  - [問題表示・解答](./display-and-answer.md)（生成された問題はここで使われる）
  - [自動採点](../grading/auto-grading.md)（生成時のサンドボックス検証は採点と同じ仕組み）
- **関連 ADR**：
