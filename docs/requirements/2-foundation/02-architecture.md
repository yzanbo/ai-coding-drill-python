...
flowchart TB
    ...
    GradWorker[apps/workers/grading<br/>Go 採点ワーカー<br/>別 VM 常駐] --> Sandbox
    ...
    GenWorker[apps/workers/generation<br/>Go 問題生成ワーカー<br/>将来] -- 抽象化レイヤ経由 --> LLM
...
| 次期（R2 以降） | 上記 + 問題生成 Worker（Go、apps/workers/generation） | 問題生成 LLM 呼び出しを Worker 化（→ [ADR 0040](../../adr/0040-worker-grouping-and-llm-in-worker.md)）。Backend API は enqueue のみに集中 |
...
#### apps/workers/grading（採点ワーカー、R0）
...
#### apps/workers/generation（問題生成ワーカー、R2 以降）

役割：
- 問題生成リクエストの非同期処理（Backend API は enqueue のみで即 202 を返す）
- LLM 呼び出し（生成プロバイダ、`prompts/generation/` 配下のプロンプト）
- Pydantic v2 経由で構造化出力をバリデーション → サンドボックスで模範解答検証 → 別プロバイダの Judge 評価 → DB 保存
...
flowchart LR
    ...
    subgraph AWS[AWS]
        ...
        subgraph EC2[EC2 専用 VM<br/>t4g.small]
            GoWorker[apps/workers/grading<br/>+ apps/workers/generation]
