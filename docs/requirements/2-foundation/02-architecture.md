...
#### 設計スタイル
APIRouter 単位の機能別レイアウト + Router / Service / Repository / ORM の 3 層分離で統一。Service はビジネスロジック + 認可 + トランザクション境界 + Pydantic 詰め替えに責務を限定し、SQLAlchemy クエリは `apps/api/app/repositories/` に集約する（→ [ADR 0044](../../adr/0044-backend-repository-pattern-adoption.md)。ROI 観点では薄い責務で delegating wrapper になりがちだが、ポートフォリオで設計パターンの理解を可視化する判断で採用）。Pydantic v2 をスキーマ・バリデーションの SSoT とし、OpenAPI 3.1 を `apps/api/openapi.json` に静的配信して TS / Go の型を生成する（→ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md)）。
...
#### 運用作法
配送保証契約（at-least-once / 可視性タイムアウト 5 分 / 指数バックオフリトライ / `state='dead'` DLQ）は [ADR 0046](../../adr/0046-job-queue-delivery-guarantees.md) を SSoT として参照。本セクションは概観のみ：
...
- Worker handler は **at-least-once 前提で冪等に実装**（payload の `submission_id` 等で重複検出、→ [ADR 0046](../../adr/0046-job-queue-delivery-guarantees.md)）
...
- LLM-as-a-Judge 呼び出し（生成と異なる別プロバイダ・別モデル、`prompts/judge/` 配下のプロンプト）。**MVP は Gemini 単独で例外保留**、R2 ベンチマーク開始時に切替（→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)）
- 実行結果（成否・失敗ケース・stdout/stderr・所要時間・Judge スコア）を Postgres に書き戻し、ジョブの `state` を更新

設計の特徴：
- 常駐プロセス、`LISTEN/NOTIFY` + ポーリングのハイブリッドでジョブ取得（配送保証契約は → [ADR 0046](../../adr/0046-job-queue-delivery-guarantees.md)）
- goroutine による並列採点
- Docker Engine と同じ VM に住み、`/var/run/docker.sock` 経由で Docker 操作（DooD 採用 / DinD 不採用 → [ADR 0045](../../adr/0045-sandbox-container-runtime-dood.md)）
...
- 構造化出力を Pydantic 由来 JSON Schema から quicktype `--src-lang schema` で生成した Go struct でバリデーション（Job キュー境界、→ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md)） → サンドボックスで模範解答検証 → 別プロバイダの Judge 評価（**MVP は Gemini 単独で例外保留**、R2 ベンチマーク開始時に切替、→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)） → DB 保存
...
採点 Worker がホスト Docker daemon の `/var/run/docker.sock` を共有して兄弟コンテナとしてサンドボックスを起動する **DooD 方式**を採用（DinD 不採用）。起動主体（DooD vs DinD vs 代替）の選択根拠は [ADR 0045](../../adr/0045-sandbox-container-runtime-dood.md) を参照。隔離強度（Docker runtime / gVisor / Firecracker）の段階強化は本 ADR と直交し、[ADR 0009](../../adr/0009-disposable-sandbox-container.md) を参照。
