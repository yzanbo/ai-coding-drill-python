...
| Judge 評価 | **生成と異なるプロバイダ**で評価し自己評価バイアス回避（[ADR 0008](../../adr/0008-custom-llm-judge.md)）。**MVP は Gemini 単独で例外保留**、R2 ベンチマーク開始時に切替（→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)）|
...
- **非決定性パラメータの既定値**（役割別、YAML で上書き可、→ [ADR 0007](../../adr/0007-llm-provider-abstraction.md)）：
  - 生成（generation / regeneration）：`temperature=0.7`（多様性確保） + JSON mode 強制
  - Judge（評価）：`temperature=0.0`（評価のブレ抑制） + JSON mode 強制
  - 上記既定値は MVP 初期値で、R7 集合的評価のキャリブレーション結果で見直す
  ```json
  {
    "title": "string",
    "description": "string",
...
決定論では測れない主観的な品質を別モデル（生成と異なる中位〜高品質モデル）で評価。**MVP 段階（R1〜R2 ベンチマーク開始前）は Gemini 単独運用**で起動確認を優先する（→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)）。
...
- **生成モデルと Judge モデルは別プロバイダ・別モデル**にする（自己評価バイアス回避）。**MVP は Gemini 単独で例外保留**、R2 ベンチマーク開始時に切替（→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)）
- **合計スコア閾値 20/25**（5 軸 × 5 点満点、かつ**各軸 3 点以上必須**）を下回ったら再生成。MVP 初期値で、R7 集合的評価のキャリブレーション結果で見直す
...
- **モデル段階利用**（具体名は実装時決定、初期想定として記載）：
  - 初回生成：低コスト・高速モデル
  - 再生成 / 評価：中位モデル（生成と異なるベンダーが望ましい。**MVP は Gemini 単独で例外保留**、R2 ベンチマーク開始時に切替、→ [ADR 0049](../../adr/0049-initial-llm-model-selection.md)）
  - 最終フォールバック：高品質モデル
