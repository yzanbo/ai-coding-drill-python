...
| サンドボックス検証 | 模範解答が**全テスト通過**しなければ即破棄（[ADR 0008](../../adr/0008-disposable-sandbox-container.md)）|
| Judge 評価 | **生成と異なるプロバイダ**で評価し自己評価バイアス回避（[ADR 0009](../../adr/0009-custom-llm-judge.md)）|
...
- **トークン削減**：
  - 各プロバイダのプロンプトキャッシュ機能（Anthropic Prompt Caching、Gemini Context Caching 等）を活用。プロバイダ固有 API への依存度はトレードオフとして受容（→ [ADR 0011: 失うもの](../../adr/0011-llm-provider-abstraction.md#失うもの受容するリスク)）
