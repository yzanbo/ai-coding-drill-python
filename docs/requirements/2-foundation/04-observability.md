...
- **必須フィールド**（R1 から実装。**後追加だと過去ログを集計できない**ため最初から固める）：
  ...
  - `body`（人間可読のメッセージ本体）
  - `service.name`（`api` / `workers-grading` / `workers-generation` / `web`）
...
- **LLM 呼び出し時の追加フィールド**（R1 から記録、後追加だと履歴クエリ不可）：
  - `llm.provider`（`anthropic` / `google` / `openai` / `openrouter` 等。[ADR 0007](../../adr/0007-llm-provider-abstraction.md) の抽象化に対応）
...
| **環境変数・API キー** | スタックトレースから自動除去（Sentry の `before_send` フックで環境変数（Python: `os.environ` / Go: `os.Getenv`）由来文字列を redact） |
...
実装：FastAPI（Python）/ Go それぞれで共通の redact ミドルウェアを `observability/` モジュール配下に置く。
...
- **DB クエリ**（`db.client.operations`、SQLAlchemy 自動計装 → [ADR 0037](../../adr/0037-sqlalchemy-alembic-for-database.md)）
- **トランザクション**（`db.transaction` を親、内部 INSERT/UPDATE を子）
- **キャッシュ操作**（`cache.get` / `cache.set`、Redis）
- **構造化出力バリデーション**（LLM 出力パース時、Worker 側で Pydantic 由来 JSON Schema (`apps/api/job-schemas/`) → quicktype 生成 Go 型、→ [ADR 0006](../../adr/0006-json-schema-as-single-source-of-truth.md)）
...
FastAPI（Producer）→ Postgres `jobs` → Go ワーカー（apps/workers/grading・apps/workers/generation、Consumer）はプロセスが分かれるため、**OTel Context をジョブペイロードに埋め込んで伝播させる**。これがないとユーザーリクエストとワーカー処理が別トレースに分断される。**ペイロードスキーマは R1 で確定するため、この方針も R1 で固める必要がある**。

→ 採用方式・代替案・トレードオフは [ADR 0010: W3C Trace Context をジョブペイロードに埋め込む](../../adr/0010-w3c-trace-context-in-job-payload.md) を参照。

#### 実装方針

1. FastAPI でジョブ INSERT 時、現在の OTel Context から **W3C Trace Context** を取り出して `payload.traceContext` に格納
2. Go ワーカーがジョブ取得時に `payload.traceContext` から OTel Context を復元
3. ワーカー側の `job.process` スパンを **FastAPI 側の親スパンにリンク**（`SpanLink` または `parent` として接続）
...
```
[POST /problems/generate] 35ms                          ← FastAPI リクエスト（enqueue のみ）
  ├─ [auth.session.verify] 3ms
  ├─ [db.transaction] 25ms
  │    ├─ [db.insert jobs] 6ms
  │    └─ [db.execute NOTIFY] 2ms
  └─ [http.response 202] 1ms

[POST /submissions] 45ms                               ← FastAPI リクエスト
  ├─ [db.transaction] 30ms
...
[job.process (id=99, queue=generation)] 1300ms          ← apps/workers/generation（同一 trace_id で連結）
  ├─ [job.acquire] 12ms
  ├─ [cache.get] 5ms
  ├─ [llm.generate (provider=anthropic, model=...)] 800ms
  │    └─ [http.client POST api.anthropic.com] 790ms
  ├─ [validate.openapi_schema] 8ms
  ├─ [sandbox.run (reference)] 250ms
  └─ [llm.judge (provider=google, model=...)] 120ms

[job.process (id=42, queue=grading)] 1850ms             ← apps/workers/grading（同一 trace_id で連結）
  ├─ [job.acquire] 12ms
...
- **コンテナ起動時間**（`runtime="docker" | "runsc" | "firecracker"` ラベル付き、[ADR 0009](../../adr/0009-disposable-sandbox-container.md) のベンチマーク継続観測）
- 実行時間分布（ヒストグラム）
- タイムアウト発生率
- メモリ使用量分布
...
| 事象 | 閾値（初期値） | 通知先 |
|---|---|---|
| LLM コストが日次予算を超過 | `> $1/day`（[01-non-functional.md](./01-non-functional.md#コスト) の月額上限 $30 を 30 日で按分） | メール |
