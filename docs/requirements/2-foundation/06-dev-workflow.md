...
| **Playwright（E2E）** | TS（Frontend）+ Backend / Mock 連携 | CI のみ（pre-push には入れない） | `apps/web/**` / `apps/api/**` / `apps/workers/grading/**` / `shared` のいずれか変更時 | `playwright test`（`web-e2e` ジョブ） | Postgres + Redis services + chromium ブラウザ install + 3 サーバ（mock GitHub / api / web）並行起動が必要で重いため pre-push 非搭載。**dev DB と完全分離した `docker-compose.test.yml`（`postgres-test :5433/ai_coding_drill_test` + `redis-test :6380`、tmpfs）に接続**し、`/_test/reset` の TRUNCATE / FLUSHDB は DB 名末尾 `_test` allowlist で dev DB に届かない構造。失敗時は `test-results/` を artifact 化（→ [ADR 0038](../../adr/0038-test-frameworks.md)） |
...
すべての commit に **header に加えて本文（body）を詳細に書く**。trivial な fix typo / format only でも本文で「何を / なぜ」を補足する。

**理由**：

- header（100 文字以内）だけでは「**何を**」しか書けず「**なぜ**」「**どう影響するか**」が抜ける
- 数ヶ月後にメンテナが `git log` を読んだとき、PR 説明や Slack ログがなくても commit 単位で判断できる情報を残す
- レビューア（採用担当者を含む）が PR を読まずに commit 履歴を辿れる
- ポートフォリオ用途で「設計判断の根拠を逐一記録している」を可視化する

**本文に含める内容**（順序自由、該当しない項目は省略可）：

| 観点 | 何を書くか |
|---|---|
| **WHAT** | 何を変えたか（header よりも具体的に、対象ファイル / クラス / 関数の粒度） |
| **WHY** | なぜ変えたか（背景・問題・採用理由・代替案を退けた根拠） |
| **HOW** | 採用した手法の要点（実装の細部ではなく設計判断レベル） |
| **影響範囲** | 他の機能・コンポーネント・API・運用への波及 |
| **関連参照** | ADR / 要件 .md / Issue / PR のリンク（→ なぜそうした判断かを追跡可能にする） |

**OK 例**：

```
feat(api): 採点ジョブ enqueue エンドポイントを追加

POST /submissions が呼ばれたら同一トランザクションで submissions レコード
INSERT + jobs テーブル INSERT + NOTIFY new_job を実行し、202 + submissionId
を即返す（ADR 0004 のジョブキュー設計）。LLM 呼び出しは Worker 側に閉じる
方針（ADR 0040）に従い、本 API はサンドボックスや Judge を一切触らない。

採点フローの全体像は docs/requirements/4-features/grading.md §採点フロー を参照。
```
...
feat(api): エンドポイント追加
```
（subject だけで body なし。何が / なぜ が不明 → コミット履歴の価値が下がる）

```
fix: typo
```
（typo の場所も理由も不明。最低でも「どのファイルの何を直したか」を body に書く）

このルールに違反する commit は レビューア / Claude が PR で指摘し、本文追記 amend または squash を依頼する。
