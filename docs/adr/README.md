...
| [0018](./0018-biome-for-tooling.md) | TypeScript のコード品質ツールに Biome を採用、設定は `apps/web/` 配下に直接配置 *(Superseded by 0033)* | Rust 製高速 / 設定統一 / 単一設定スキャン（Frontend 用途として継続採用） |
...
| [0024](./0024-syncpack-package-json-consistency.md) | モノレポ内 package.json の整合性を syncpack で機械強制（versionGroups / semverGroups の最小限ルールセット）*(Superseded by 0033, 0036)* | sameRange / `^` 統一 / workspace:* 強制（Frontend 用途として継続採用、Python 側は uv lockfile で代替、apps/web/ 配下に再配置） |
...
- 例外：**採用判断は維持しつつ前提（言語スタックや配置範囲）が大きく変わったケース**も `Superseded by NNNN` として扱う。本文冒頭の Note で「採用は維持・XX に再配置 / ルールセット縮小」を明記する。例：[ADR 0018](./0018-biome-for-tooling.md)（Biome 採用は維持、設定を `apps/web/` 配下に閉じる）/ [ADR 0024](./0024-syncpack-package-json-consistency.md)（syncpack 採用は維持、`apps/web/.syncpackrc.ts` に再配置・ルール 3 件に縮小）
