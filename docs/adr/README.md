...
| [0017](./0017-drizzle-orm-over-prisma.md) | ORM に Drizzle を採用（Prisma 不採用）*(Superseded by 0033)* | 型推論 / 生 SQL 親和性 / コールドスタート（移行軌跡として保持） |
| [0018](./0018-biome-for-tooling.md) | TypeScript のコード品質ツールに Biome を採用、設定はリポジトリルートに直接配置 *(Superseded by 0033)* | Rust 製高速 / 設定統一 / 単一設定スキャン（移行軌跡として保持） |
...
| [0023](./0023-turborepo-pnpm-monorepo.md) | モノレポツールに Turborepo + pnpm workspaces を採用 *(Superseded by 0033)* | ビルドキャッシュ / 並列実行（Python 用モノレポ管理に置換予定） |
| [0024](./0024-syncpack-package-json-consistency.md) | モノレポ内 package.json の整合性を syncpack で機械強制（versionGroups / semverGroups の最小限ルールセット）*(Superseded by 0033)* | sameRange / `^` 統一 / workspace:* 強制（Python 用依存整合性に置換予定） |
