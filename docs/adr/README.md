...
| [0045](./0045-sandbox-container-runtime-dood.md) | 採点 Worker のサンドボックスコンテナ起動方式に DooD を採用、DinD は不採用 | ホスト docker.sock 共有 / セキュリティモデル / Worker 専用 VM |
| [0046](./0046-job-queue-delivery-guarantees.md) | ジョブキューの配送保証契約（at-least-once / 可視性タイムアウト / 指数バックオフリトライ / `state='dead'` DLQ） | Producer / Worker の契約 / NOTIFY は best-effort / 冪等性は Worker 責務 |
| [0047](./0047-session-store-on-redis.md) | セッションストアに Redis を採用（Postgres セッションテーブル / JWT を不採用） | Cookie + 不透明 ID / SameSite=Lax / double submit cookie / 認証チェック高頻度 |
| [0048](./0048-soft-delete-for-user-facing-tables.md) | ユーザー向けテーブル（users / problems / submissions）にソフトデリートを採用（`deleted_at` カラム方式） | 参照整合性維持 / 本人視点と集計視点の両立 / PII 匿名化と分離 |
| [0049](./0049-initial-llm-model-selection.md) | MVP の初期 LLM プロバイダは Gemini 単独（`gemini-3.5-flash` を全ロールに割当）、R2 ベンチマークで別ベンダー Judge に切替予定 | ポートフォリオ用途 / 無料枠優先 / 抽象化レイヤで切替容易 / ADR 0008 例外運用 |
...
| ADR | タイトル | キーワード |
|---|---|---|
| [0014](./0014-nestjs-for-backend.md) | バックエンド API に NestJS を採用（Hono / Fastify / Express 不採用）*(Superseded by 0033, 0034)* | DI / Module / Guard / レイヤード設計（移行軌跡として保持） |
...
| [0042](./0042-frontend-data-fetching-tanstack-query.md) | フロントエンドのデータフェッチに TanStack Query を採用（R1-4 で導入） | サーバ状態管理 / ポーリング / Hey API 公式プラグイン / SWR 不採用 |
| [0043](./0043-frontend-ui-styling-stack.md) | フロントエンドの UI / スタイリングスタックに Tailwind CSS v4 + shadcn/ui + React Hook Form + Zod を採用 | CSS-first 設定 / Radix ベース / 自前所有コンポーネント / Hey API 生成 Zod と整合 |
...
| **R3** サンドボックス強化 | [0009](./0009-disposable-sandbox-container.md) / [0045](./0045-sandbox-container-runtime-dood.md) |
