...
- GitHub Actions に至っては、R0 で同時導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）と組み合わせると **メンテナが手で SHA を追従するのは現実的に不可能**（40 文字のハッシュを毎週手で書き換えるのは破綻する）
...
| 候補 | 概要 | 採用しなかった理由 |
|---|---|---|
| A. 手動更新 | 必要時に `pnpm up` / Action 手書き更新 | R0 で同時導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）との組み合わせが破綻（40 文字 SHA を手で追従不可）。脆弱性追従が属人化 |
...
- **R0 で同時導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）が実用可能になる**：人間が SHA を追従する負担なしに不変参照を維持できる
