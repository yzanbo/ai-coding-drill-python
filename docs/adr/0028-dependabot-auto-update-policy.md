# 0028. 依存関係の自動更新ポリシー（Dependabot）

- **Status**: Accepted
- **Date**: 2026-05-10 <!-- Python pivot（ADR 0033）に伴い pip エコシステムを監視対象に追加 -->
- **Decision-makers**: 神保 陽平

> **Note**：Python pivot（[ADR 0033](./0033-backend-language-pivot-to-python.md)）+ ADR 0026 に伴い、監視対象に **`pip`（uv 経由 / Python 依存）** を追加した。Dependabot の `pip` エコシステムは PEP 621 準拠の `pyproject.toml` を直接読むため、uv が管理する `apps/api/pyproject.toml` の依存解析・PR 生成が可能（lockfile は uv 側で再生成）。
...
- **npm（pnpm 管理）**：`apps/web/pnpm-lock.yaml` 経由で Frontend 依存ツリー全体（→ [ADR 0036](./0036-frontend-monorepo-pnpm-only.md)）
- **pip（uv 管理、Python pivot 後）**：`apps/api/pyproject.toml` + `apps/api/uv.lock` 経由で Backend 依存ツリー全体（→ [ADR 0033](./0033-backend-language-pivot-to-python.md) / [ADR 0035](./0035-uv-for-python-package-management.md)）

放置すると以下の問題が起きる：

- セキュリティ脆弱性のパッチが適用されない
- メジャーバージョン跨ぎでまとめて移行する羽目になり、移行コストが膨らむ
- GitHub Actions に至っては、同 R0-6 でセット導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）と組み合わせると **メンテナが手で SHA を追従するのは現実的に不可能**（40 文字のハッシュを毎週手で書き換えるのは破綻する）
...
**Dependabot を採用し、`.github/dependabot.yml` で github-actions / npm / pip の週次自動 PR を有効化する。** 以下のポリシーを SSoT として `.github/dependabot.yml` に集約する：

| 項目 | 設定 | 理由 |
|---|---|---|
| 監視対象 | `github-actions` + `npm`（apps/web）+ `pip`（apps/api、uv 経由） | リポジトリ内の依存全種類（Python pivot で pip を追加、→ [ADR 0033](./0033-backend-language-pivot-to-python.md) / [ADR 0026](./0026-github-actions-incremental-scope.md)） |
| 頻度 | `weekly`（月曜 06:00 Asia/Tokyo） | 週初に PR 集約 → 週内レビュー可能 |
| メジャー更新（npm / pip） | `ignore` で除外 | API 互換性が崩れることが多く、リリースノート確認・コード修正・テスト追加が必要 |
| メジャー更新（github-actions） | 許可（自動 PR 化） | SHA ピン止め（→ ADR 0027）の追従が滞ると前提が崩れる。breaking change は PR ごとに人間判断 |
| グループ化（npm） | `@types/*` / `@biomejs/*` + `biome` / `@commitlint/*` + `commitlint` | 関連パッケージの版ずれを防ぎ PR 数も削減 |
| グループ化（pip） | `ruff`（lint+format で版ずれ不可）/ `pyright` 単独 / `pytest` + `pytest-*` プラグイン群 | サブパッケージ間の API 不一致防止 |
| コミットメッセージ | `prefix: build` + `include: scope` | commitlint 規約（`type(scope): subject`）準拠 |
| PR 上限 | github-actions 5 / npm 10 / pip 10 | グループ外の単発 PR で詰まらない余裕 |
...
### メジャー更新の扱いをエコシステム別に分ける理由

メジャー更新の扱いは `npm` / `pip` と `github-actions` で非対称：

| エコシステム | メジャー更新の自動 PR 化 | 理由 |
|---|---|---|
| `npm` / `pip` | **除外（ignore）** | アプリケーション本体のランタイム依存はメジャー更新で API 互換性が崩れることが多く、リリースノート確認・コード修正・テスト追加が必要。機械的に PR 化すると毎週同じ内容で上がり close する手間がノイズ化する。必要なときに人間が `pnpm up <pkg>@latest` / `uv add <pkg>@latest` で明示的・意図的に取り込む |
| `github-actions` | **許可（自動 PR 化）** | GitHub 公式 Action はメジャー版が比較的短い間隔で続くことがある（actions/checkout v5 → v6 が数ヶ月）。メジャー版の変更内容は Node ランタイム要件の更新等が中心で、`with:` 引数の互換性に影響しない範囲に収まることが多い。`ignore` で止めると SHA ピン止めの追従が滞り、[ADR 0027](./0027-github-actions-sha-pinning.md) の前提（Dependabot で SHA を更新し続ける）が崩れる。**人間が PR ごとに breaking change の有無を確認・マージ判断する**運用とする |

`.github/dependabot.yml` の実装はこの方針通り：`github-actions` ブロックには `ignore` を置かず、`npm` / `pip` ブロックにのみ `version-update:semver-major` を `ignore` で除外する。

minor / patch は両エコシステムで自動 PR 化される（脆弱性修正の追従が止まることはない）。

### github-actions メジャー更新 PR のレビュー観点

メジャー更新 PR が来た場合に確認すべき項目：
...
| 候補 | 概要 | 採用しなかった理由 |
|---|---|---|
| A. 手動更新 | 必要時に `pnpm up` / Action 手書き更新 | 同 R0-6 で導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）との組み合わせが破綻（40 文字 SHA を手で追従不可）。脆弱性追従が属人化 |
...
- **同 R0-6 でセット導入する SHA ピン止め（→ [ADR 0027](./0027-github-actions-sha-pinning.md)）が実用可能になる**：人間が SHA を追従する負担なしに不変参照を維持できる
...
- **PR 数が週 20 件を超えるようになったら**（npm + pip + github-actions 合算）：グループ化対象を増やすか、`open-pull-requests-limit` を見直す
- **lockfile コンフリクトが頻発するようになったら**（pnpm-lock.yaml / uv.lock 双方）：Dependabot の rebase 戦略（`rebase-strategy: auto` 等）を再検討、または `cooldown` で更新タイミングを分散
- **npm / pip メジャー更新の遅延が問題化したら**：除外を解除し、人間レビュー前提で自動 PR 化する
- **github-actions のメジャー更新 PR がレビュー負荷になった場合**：Action ごとに `dependency-name` 単位で `version-update:semver-major` を ignore する（例：特定 Action だけ major を止める）
...
- [ADR 0021](./0021-r0-tooling-discipline.md)：補完ツールを R0 から導入（Dependabot もこの系譜）
- [ADR 0026](./0026-github-actions-incremental-scope.md)：GitHub Actions の段階拡張（Dependabot は R0 構成の一部）
- [ADR 0025](./0025-github-actions-as-ci-cd.md)：CI/CD ツールに GitHub Actions を採用
