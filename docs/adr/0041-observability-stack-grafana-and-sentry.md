...
観測性の **Three Pillars**（ログ / トレース / メトリクス）を Grafana スタックで、**エラー追跡**は補助的役割として Sentry を採用する。Three Pillars は「定常的にアプリの挙動を観測する」担当、Sentry は「例外発生時の即時通知と詳細スタックトレース」担当で役割が異なる。

| 分類 | 観測対象 | 採用ツール | 役割 |
|---|---|---|---|
| **Three Pillars** | **ログ** | **Grafana Loki** | 構造化ログの集約・検索 |
| **Three Pillars** | **トレース** | **Grafana Tempo** | 分散トレース（[ADR 0010](./0010-w3c-trace-context-in-job-payload.md) の W3C Trace Context 受信先） |
| **Three Pillars** | **メトリクス** | **Prometheus** | 数値メトリクスの収集・保存・クエリ |
| **Three Pillars** | **可視化 UI** | **Grafana** | 上記 3 系統を 1 画面で統合表示 |
| **補助** | **エラー追跡** | **Sentry**（無料枠） | フロント / バックのエラー集約・スタックトレース・リリーストラッキング |
