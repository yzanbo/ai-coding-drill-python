...
調査の結果、**GitHub の smart HTTP Git プロトコル v2 サーバは `--shallow-exclude` の SHA 指定における acknowledgments 機能が未対応**であることが判明（GitHub のフェッチプロトコル実装は OSS Git のフル機能を網羅していない）。tag 名指定（`--shallow-exclude=v1.0`）は通るが、SHA 指定は通らないという挙動。

これは GitHub 側の仕様であり、利用者側からは制御不能。
...
- name: "[PR] Deepen until base reachable"
  ...
  run: |
    ...
    until git cat-file -e "${BASE_SHA}^{commit}" 2>/dev/null; do
...
| D. `--shallow-exclude=<tag-name>` | タグ名指定なら GitHub でも通る | base 側にタグが付いていない一般的な PR では適用不能 |
| E. **iterative deepen（採用）** | base 到達まで `--deepen=20` ループ | プロトコル依存なし、規模に応じた取得量、決定的 |
| F. ローカル `git fetch` を使わず GitHub API で base..head を取得 | `gh api` 等でメッセージ列を直接取得 | commitlint は git ログを期待する設計、API 経由は変換層が必要で複雑化 |
...
- **ループの暴走防止が無い**：理論上 `git fetch` が永遠に何も追加しない場合は無限ループになる。現実には `--deepen` が履歴の根に到達すれば必ず base が含まれるため発生しないが、安全弁として最大ループ回数を設けるかは将来検討
...
- **無限ループ事故が発生**：最大ループ回数（例：10 回 = 200 コミット）を設けて fail-fast に切り替え
...
- [GitHub Docs: Smart HTTP Git protocol v2 limitations](https://docs.github.com/en/get-started/git-basics/git-internals)
