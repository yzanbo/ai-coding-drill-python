...
が **別々の trace_id** として記録され、Tempo / Jaeger 上で連結された 1 本のトレースとして可視化できない。これでは [06_observability.md: 可視化例](../requirements/base/06_observability.md#可視化例) の「ユーザーリクエストから採点完了までを 1 トレースで追う」が成立しない。

ジョブペイロードのスキーマは Phase 1 で確定し、確定後は JSON Schema を SSoT として TS / Go 両言語の型を自動生成する（→ [ADR 0014](./0014-json-schema-as-single-source-of-truth.md)）。**ペイロード構造を後から変更すると、進行中ジョブのマイグレーションと両言語のコード追従が必要**になるため、Phase 1 着手前にトレース連携の方針を決めておく必要がある。
...
- 全ジョブ種別の JSON Schema に `traceContext` フィールドを **必須**として定義（[09_data_model.md: ジョブペイロードのスキーマ](../requirements/base/09_data_model.md#ジョブペイロードのスキーマ)）
...
- Phase 1 で**最初のジョブ INSERT を書く時点で実装**する（後追加だと進行中ジョブのマイグレーション必要）
...
| 伝播せず、ジョブ実行を独立トレースとする | 実装が最も簡単 | [06_observability.md: 可視化例](../requirements/base/06_observability.md#可視化例) のような連結可視化が成立せず、観測性要件を満たさない。「採点が遅かった原因を遡る」分析が困難になる |
| 自前フォーマット（独自 JSON 構造） | 完全な制御 | OTel SDK に組み込まれた `propagator` を使えず、注入・抽出処理を自前実装。標準仕様を逸脱するメリットが無い |

## Consequences（結果・トレードオフ）

### 得られるもの
- **NestJS リクエスト → Go ワーカー処理を単一 trace_id で連結可視化**できる（[06_observability.md: 可視化例](../requirements/base/06_observability.md#可視化例)）
- **トレース基盤の差し替え可能性**：W3C 標準なので Tempo / Jaeger / Datadog / New Relic 等のいずれでも動作（[ADR 0011](./0011-llm-provider-abstraction.md) と同じ「ベンダ非依存」哲学と整合）
...
- [06_observability.md: トレース](../requirements/base/06_observability.md#トレース)
- [06_observability.md: プロセス境界をまたぐトレース連携](../requirements/base/06_observability.md#プロセス境界をまたぐトレース連携phase-1-で必須)
- [09_data_model.md: ジョブペイロードのスキーマ](../requirements/base/09_data_model.md#ジョブペイロードのスキーマ)
...
### 4. 06_observability.md / 09_data_model.md への反映

本補足の 1〜3 は実装着手時に [06_observability.md](../requirements/base/06_observability.md) の主要スパン記述および [09_data_model.md](../requirements/base/09_data_model.md) の `traceContext` 共通フィールド記述に反映する（Phase 1 の実装作業に含める）。
