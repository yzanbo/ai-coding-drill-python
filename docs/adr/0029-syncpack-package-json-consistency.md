...
- **CI**：`.github/workflows/ci.yml` に独立した `syncpack` ジョブを追加。Biome / typecheck / commitlint と並列実行。pre-commit を `--no-verify` で skip された場合の最終 gate として機能（多層防御）
- **lefthook pre-commit**：`syncpack lint` を `glob: "package.json"` 付きで接続。`package.json` が staged の時だけ起動し、通常のコード編集（`.tsx` 等）では走らない。**lint のみ**接続し、自動修正（`fix-mismatches`）は接続しない（後述「自動 fix を pre-commit で走らせない理由」）
- **設定の所在**：ルート直接配置（`.syncpackrc.ts`）。配置方針の全体像は [packages/config/README.md](../../packages/config/README.md) を参照
...
- [ADR 0017: W3C Trace Context をジョブペイロードに埋め込む](./0017-w3c-trace-context-in-job-payload.md)：同じ「遅延すると将来コストが膨張する判断」シリーズ
- [ADR 0018: 補完ツールを R0 から導入](./0018-phase-0-tooling-discipline.md)：同じく遅延不可逆判断シリーズ、メタ方針の確立元
