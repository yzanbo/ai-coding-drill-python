# 0030. CI Required status checks を集約ジョブ `ci-success` で 1 本化し main マージ時に強制
...
### 2. Ruleset 側：`protect-main` に `required_status_checks` を統合（main のみ強制）

既存ルールセット `protect-main` に `required_status_checks` rule を追加する。target は変更せず `~DEFAULT_BRANCH`（main のみ）のまま。
...
- main への PR マージは `ci-success` 緑必須（→ 壊れたコードが main に入らない）
- main への直接 push は引き続き `pull_request` rule で禁止
- feature ブランチへの直接 push / PR マージは無検査（→ 課題 3 のチキン＆エッグを回避）
- スタック PR の責任シフトリスクは PR レビュー時の人間的確認で対処（base ブランチの CI 状態を確認することを慣行とする）

### ルールセットを別ファイルに分離しない判断

当初は `protect-main`（main 固有縛り）と `require-ci-pass`（全ブランチ CI 強制）の 2 ルールセットに責任分離する案を検討したが、target がいずれも `~DEFAULT_BRANCH` に収束した結果、分離の根拠が消失した。1 つのルールセットに集約し、main 保護に関する全ルールが 1 か所で読めるようにする。
