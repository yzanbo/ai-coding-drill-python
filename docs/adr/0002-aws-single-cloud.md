...
| Vercel + Fly.io + Supabase 等の SaaS 寄せ集め | 各 SaaS の無料枠で運用 | クラウド選定の見せ場が薄い、IAM/IaC の練習機会が減る（コアの設計判断は AWS で行いつつ、Frontend / Redis のみ無料枠 SaaS を活用する判断は別途 [ADR 0007](./0007-upstash-redis-over-elasticache.md) / [ADR 0032](./0032-vercel-for-frontend-hosting.md) で正当化） |
