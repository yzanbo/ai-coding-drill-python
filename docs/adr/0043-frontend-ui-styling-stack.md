...
apps/web（Next.js 16+ / React 19 / TypeScript、→ [ADR 0036](./0036-frontend-monorepo-pnpm-only.md)）で画面・フォームを構成する技術を確定する必要がある。R1-4（[F-03: 問題表示・解答入力](../requirements/4-features/F-03-problem-display-and-answer.md)）でログイン・問題一覧・解答画面・フォームが本格化するため、その手前で UI プリミティブ・スタイリング・フォーム handling・入力検証の 4 つを束ねて凍結したい。
...
- [F-01: GitHub OAuth ログイン](../requirements/4-features/F-01-github-oauth-auth.md) — shadcn/ui + RHF + Zod を導入する最初の画面
- [F-03: 問題表示・解答入力](../requirements/4-features/F-03-problem-display-and-answer.md) — 解答フォームの本実装
