...
R0 時点で CI ジョブは Python pivot（[ADR 0033](./0033-backend-language-pivot-to-python.md)）に伴い拡張され、`commitlint` / `biome` / `typecheck` / `ruff` / `pyright` / `pip-audit` / `syncpack` / `knip` / `golangci-lint`（skeleton）の 9 本となる予定（[ADR 0026](./0026-github-actions-incremental-scope.md) 拡張版）。今後さらに `test`（pytest / Vitest / go test）/ `build` / `e2e` / `migration-drift` 等が言語ごとに増えていく見込み。**3 言語に対し言語ごと 3〜5 ジョブが順次追加される構造で、ジョブ追加のたびに GitHub UI 側の Ruleset 編集を求める運用は確実に漏れる**。
