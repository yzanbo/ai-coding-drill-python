...
- **Status**: Accepted
- **Date**: 2026-05-09 <!-- Python pivot に伴う R0 ジョブ拡張に追従 -->
...
R0 時点で CI ジョブは Python pivot（[ADR 0033](./0033-backend-language-pivot-to-python.md)）に伴い拡張され、`commitlint` / `biome` / `typecheck` / `ruff` / `pyright` / `pip-audit` / `deptry` / `syncpack` / `knip` / `golangci-lint`（skeleton）/ `go-mod-tidy-check`（skeleton）の 11 本となる予定（[ADR 0026](./0026-github-actions-incremental-scope.md) 拡張版）。今後さらに `test`（pytest / Vitest / go test）/ `build` / `e2e` / `migration-drift` 等が言語ごとに増えていく見込み。**3 言語に対し言語ごと 3〜5 ジョブが順次追加される構造で、ジョブ追加のたびに GitHub UI 側の Ruleset 編集を求める運用は確実に漏れる**。
...
- **3 言語ポリグロット構成での価値が顕著**：[ADR 0003](./0003-phased-language-introduction.md) の Python / TS / Go の 3 言語構成では言語ごとに lint / 型 / 脆弱性スキャン / テスト の数本ずつのジョブが必要となり、Required status checks の数が早期に二桁に達する。集約ジョブパターンが無いと運用負荷が線形膨張する
...
- [ADR 0033: バックエンドを Python に pivot](./0033-backend-language-pivot-to-python.md)（R0 ジョブ数拡張の契機）
- [ADR 0021: 補完ツールを R0 から導入](./0021-r0-tooling-discipline.md)（R0 ジョブの根拠ツール群）
- [ADR 0039: タスクランナー兼 tool 版数管理に mise を採用](./0039-mise-for-task-runner-and-tool-versions.md)（各 needs ジョブを `mise run <task>` で統一する前提）
