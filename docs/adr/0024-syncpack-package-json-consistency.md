...
[ADR 0022: 設定ファイル形式の選定方針](./0022-config-file-format-priority.md) の「自由選択時は TS > JSONC > YAML の優先順位」に従う。具体的には：
...
- コメントで「なぜこのルールがあるか」を残せることが SSoT としての価値を高める（ADR 0022 の前提原則「設定ファイルには『なぜ』をインラインコメントで残す」と一致）
