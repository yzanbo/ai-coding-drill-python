...
- 旧 `packages/config/` 配下に置く案も検討したが、設定発見のために `--config` フラグが必要になり運用が煩雑化（`packages/config/` は ADR 0036 で廃止済み）
- ADR 0036 拡張により、現在は `apps/web/.syncpackrc.ts` 配置（apps/web 内の単一 package.json でも有効な縮小ルールセットで運用）。Frontend が単一 app に縮退したため、root 配置でも apps/web 内配置でもツール挙動は同等
