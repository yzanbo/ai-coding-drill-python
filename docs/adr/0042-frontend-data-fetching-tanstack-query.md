...
Next.js 16+（App Router）でフロントエンドを実装する際、サーバ状態（API レスポンス）の取得・キャッシュ・再取得をどう扱うかを決める必要がある。本サービスは非同期ジョブ中心（問題生成 / 採点）で、解答送信後に **採点結果をポーリング** する UX が R1-5（[自動採点](../requirements/4-features/grading.md)）で必須となる。
...
- [自動採点](../requirements/4-features/grading.md)
