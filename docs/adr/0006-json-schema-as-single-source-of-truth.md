...
> **再設計の経緯**：本 ADR は当初「JSON Schema を SSoT」として `packages/shared-types/schemas/` に置く設計だったが、**Python pivot（[ADR 0033](./0033-backend-language-pivot-to-python.md)、2026-05-09）直後に Pydantic SSoT + 境界別 2 伝送路に再設計**した。詳細な経緯は §Context の「Python pivot に伴う SSoT 戦略の見直し」を参照。
...
- **drift 検出は lefthook + CI の二軸**：lefthook で早期 feedback（`apps/api/app/schemas/` の変更時のみ走らせ、artifact 未ステージなら abort）、CI で迂回不可の最終ゲート（`git diff --exit-code` で fail-closed）を担う。両者の役割分担・起動条件・配線タイミングの運用詳細 SSoT は [06-dev-workflow.md: drift 検出（lefthook + CI 二軸）](../requirements/2-foundation/06-dev-workflow.md#drift-検出lefthook--ci-二軸)（運用ルール型 ADR、→ `.claude/rules/docs-rules.md` §2）。本 ADR は採用根拠（§Why）と代替案（§Alternatives Considered）を扱う
...
- **drift 検出を lefthook と CI の二軸で維持する保守コスト**：lefthook hook と GitHub Actions job の両方を `apps/api/` 着手時に整備する必要（運用詳細は [06-dev-workflow.md: drift 検出（lefthook + CI 二軸）](../requirements/2-foundation/06-dev-workflow.md#drift-検出lefthook--ci-二軸)、CI 配線時期は [ADR 0026](./0026-github-actions-incremental-scope.md) の R3 拡張項目）
