...
```
ブラウザ → FastAPI（Python）── Postgres jobs INSERT + NOTIFY ──→ Go ワーカー → サンドボックス
```

OpenTelemetry の自動計装は **同一プロセス内のスパン親子関係**しか自動的には伝播しないため、上記フローでは：

- FastAPI のリクエスト処理（`POST /submissions`）と
- Go ワーカーの採点処理（`job.process`）

が **別々の trace_id** として記録され、Grafana Tempo（→ [05-runtime-stack.md: 観測性](../requirements/2-foundation/05-runtime-stack.md#観測性)）上で連結された 1 本のトレースとして可視化できない。これでは [04-observability.md: 可視化例](../requirements/2-foundation/04-observability.md#可視化例) の「ユーザーリクエストから採点完了までを 1 トレースで追う」が成立しない。

ジョブペイロードのスキーマは R1 で確定し、確定後は Pydantic を SSoT として境界別 2 伝送路で各言語に展開する（HTTP API 境界 → Hey API で TS、Job キュー境界 → quicktype で Go、→ [ADR 0006](./0006-json-schema-as-single-source-of-truth.md)）。**ペイロード構造を後から変更すると、進行中ジョブのマイグレーションと各言語のコード追従が必要**になるため、R1 着手前にトレース連携の方針を決めておく必要がある。
...
- 全ジョブ種別の Pydantic Job payload モデルに `traceContext` フィールドを **必須**として定義（[01-data-model.md: ジョブペイロードのスキーマ](../requirements/3-cross-cutting/01-data-model.md#ジョブペイロード共通フィールドtracecontext)）
- FastAPI（Producer）：ジョブ INSERT 時、現在の OTel Context から `traceparent` / `tracestate` をシリアライズして `payload.traceContext` に格納
- Go ワーカー（Consumer）：ジョブ取得時、`payload.traceContext` から OTel Context を復元し、ワーカー側のスパン（`jobs process`）を FastAPI 側の親スパンに **SpanLink** で接続（理由は下記）
...
- Producer（FastAPI の `POST /submissions`）：INSERT 後 ~50ms でレスポンス返却 → スパン close
...
| スパン側 | スパン名 | 必須属性 |
|---|---|---|
| Producer（FastAPI） | `jobs send` | `messaging.system="postgresql"`、`messaging.destination.name="jobs"`、`messaging.message.id="<jobId>"`、`messaging.operation="publish"` |
| Consumer（Go ワーカー） | `jobs process` | 上記同属性 + `messaging.operation="process"` |

これにより Grafana Tempo 上で「`jobs` キュー全体のスループット」「キューイング遅延分布」「特定 `messaging.message.id` でのジョブ追跡」が可能になる。
...
1. **観測性要件（リクエスト → 採点完了の連結可視化）の構造的成立**
   - FastAPI リクエストと Go ワーカー処理が別 trace_id だと「採点が遅かった原因を遡る」分析が困難
   - W3C Trace Context をペイロードに埋め込むことで、プロセス・言語境界をまたいだ単一トレース連結が成立する
2. **W3C 標準採用によるベンダ非依存性**
   - 採用するトレースバックエンドは **Grafana Tempo**（観測性スタック全体の選定根拠は → [ADR 0041](./0041-observability-stack-grafana-and-sentry.md)）
   - W3C 標準採用により、将来 Jaeger / Datadog / New Relic 等への差し替え可能性を構造的に確保
   - LLM プロバイダ抽象化（→ [ADR 0007](./0007-llm-provider-abstraction.md)）と同じ「ベンダ非依存」哲学と整合
3. **OTel SDK 標準 API（propagator）が使える**
   - Python / Go 両言語で `inject` / `extract` が標準提供され、シリアライズ・デシリアライズの自前実装が不要
...
5. **遅延の不可逆性（[ADR 0021](./0021-r0-tooling-discipline.md) のメタ方針と整合）**
   - ペイロード構造を後から変更すると進行中ジョブのマイグレーションと各言語のコード追従が必要
   - Pydantic SSoT + 境界別 2 伝送路（→ [ADR 0006](./0006-json-schema-as-single-source-of-truth.md)）として R1 着手前に確定させる方が後コストを構造的に防げる
...
### 得られるもの
- **FastAPI リクエスト → Go ワーカー処理を単一 trace_id で連結可視化**できる（[04-observability.md: 可視化例](../requirements/2-foundation/04-observability.md#可視化例)）
- **トレース基盤は Grafana Tempo を採用**（観測性スタック全体の選定根拠は → [ADR 0041](./0041-observability-stack-grafana-and-sentry.md)）。W3C 標準採用により Jaeger / Datadog / New Relic 等への差し替えも可能（[ADR 0007](./0007-llm-provider-abstraction.md) と同じ「ベンダ非依存」哲学と整合）
- **OTel SDK 標準の `propagator` API（Python / Go 両方）**を使えるため、シリアライズ・デシリアライズの自前実装不要
...
- **必須フィールド化により、ペイロード生成側の実装ミスでバリデーションエラーになりうる**：FastAPI 側の Producer ヘルパー関数を一箇所に集約し、忘れ防止する
- ペイロードに常に Trace Context が含まれるため、**ログ / DB ダンプ時の PII / 機密情報の取り扱いに含める**必要がある（とはいえ Trace Context 自体は機密ではない）

### R1〜R3 期間の暫定挙動（OTel SDK 未導入）
- 本 ADR の Decision は OTel SDK が結線された状態を前提にしているが、R1〜R3 では OTel SDK 自体が未導入のため `traceparent` に有効値を入れられない
- 暫定として `traceContext.traceparent: str | None = None` を許容し、FastAPI Producer は `None` を入れて送る。Go ワーカー Consumer は **`traceparent` が `None` の場合に親なしの新規 root span を発行**する
- 空文字 `""` を sentinel に使わないのは W3C 仕様で無効書式となり、将来 parser を差し込んだ際に「無効値」と「未指定」を区別できなくなるため
- **R4「観測性」着手時に OTel SDK を結線し、`opentelemetry.propagate.inject(...)` で常時有効値を詰める形に切り替える**（`traceparent` は再度 non-null 前提に戻す）
...
- [ADR 0006: Pydantic を SSoT に、境界別 2 伝送路で各言語に展開](./0006-json-schema-as-single-source-of-truth.md)
- [ADR 0041: 観測性スタックに Grafana 系 + Sentry を採用](./0041-observability-stack-grafana-and-sentry.md) — 本 ADR が決めた Trace Context の受信先を確定する ADR
