...
| ✅ 実装済み | アクション SHA ピン止め | `actions/checkout` / `actions/setup-node` / `pnpm/action-setup` を 40 文字 SHA + バージョンコメント形式で固定（タグ書き換え攻撃対策） → [ADR 0027](./0027-github-actions-sha-pinning.md) |
| ✅ 実装済み | Dependabot | `.github/dependabot.yml` で github-actions / npm の週次自動更新 PR。SHA ピン止めとセットで運用 → [ADR 0028](./0028-dependabot-auto-update-policy.md) |
...
- **commitlint / Biome / typecheck は ADR 0021 で既に R0 採用済み**：ローカル（lefthook）でしか動いていない状態だと、フックを bypass された PR を受け入れてしまう。CI 化して初めて規約遵守が保証される
...
- [ADR 0018](./0018-biome-for-tooling.md)：Biome を採用
- [ADR 0021](./0021-r0-tooling-discipline.md)：補完ツールを R0 から導入
- [ADR 0001](./0001-requirements-as-5-buckets.md)：要件定義書の 5 バケット時系列構造
- [ADR 0025](./0025-github-actions-as-ci-cd.md)：CI/CD ツール選定（GitHub Actions 採用）
