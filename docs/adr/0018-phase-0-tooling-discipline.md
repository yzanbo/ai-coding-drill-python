# 0018. 補完ツール（Knip / lefthook / commitlint / syncpack）を R0 から導入
...
なお Biome は [ADR 0013](./0013-biome-for-tooling.md) で R0 採用が確定済みだが、本 ADR では Biome 以外の補完ツール群について **R0 導入の方針自体** を決定対象とする。
...
2. **YAGNI 原則の正しい適用範囲を明示**
   ...
   - 同様の性質を持つ判断（観測性の `traceContext` → ADR 0017、PII マスキング、テストフレームワーク選定）にも転用できるメタ方針
...
- [ADR 0013: TypeScript のコード品質ツールに Biome を採用、設定はリポジトリルートに直接配置](./0013-biome-for-tooling.md)
- [ADR 0020: Go のコード品質ツール（gofmt + golangci-lint）](./0020-go-code-quality.md)
- [ADR 0021: Python のコード品質ツール（ruff、型チェッカーは R7 着手時決定）](./0021-python-code-quality.md)
