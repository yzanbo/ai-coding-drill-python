# 0001. 要件定義書を「時系列 × 変更頻度」の 5 バケット構造に再編
...
1. **「読む順序」と「書く順序」の分離が必要**
   - 読み手は抽象 → 具体（vision → roadmap）、書き手はバケット間を往復する
   - ディレクトリ番号（1〜5）で読む順序を表現し、書く順序は README の Step 1〜9 で別表として明示することで両立
   - 単一連番（旧 base/01〜10）ではこの両立が不可能
2. **変更頻度で物理分離することで git diff のノイズが減る**
...
4. **`3-cross-cutting/` を独立バケットにする必然性**
   ...
   - 4 バケット案（3-cross-cutting を 2-foundation に統合）では更新頻度が異なるものが同居し、git diff が乱雑になる
5. **アジャイル運用・カスタムコマンドとの整合**
   - 4-features がスプリント実装単位、5-roadmap がバックログ管理単位として `/new-requirements` カスタムコマンドの前提と一致
