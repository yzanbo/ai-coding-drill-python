...
> **Note**：本 ADR は `v1.0.0-typescript` 時点の判断軌跡として保持する。現行 ORM は **SQLAlchemy 2.0（async）+ Alembic** を採用しており、本文の Drizzle 記述は適用されない。最新方針は [ADR 0037](./0037-sqlalchemy-alembic-for-database.md) を参照。
...
- 同じ DB をジョブキューとしても使う（→ [ADR 0004](./0004-postgres-as-job-queue.md)）
...
- バックエンドは NestJS（→ [ADR 0014](./0014-nestjs-for-backend.md)）。Service が DB アクセスを直接行う方針で、Repository レイヤは設けない
...
1. **Postgres 固有機能を素直に書ける**
   ...
   - Prisma は独自 DSL ベースで、これらは `$queryRaw` への退避が頻発し、ジョブキュー設計（→ ADR 0004）と相性が悪い
2. **同一トランザクションでの混合操作**
...
4. **軽量ランタイム（コールドスタート優位）**
   ...
   - ECS Fargate 最小タスク 1 構成（→ ADR 0014）と相性が良い
5. **JSON Schema SSoT 戦略との責務分離**
   - Drizzle は「DB スキーマ」、JSON Schema は「ペイロードスキーマ」と SSoT を別レイヤで分離（→ ADR 0006）
   - Prisma の `.prisma` DSL は SSoT が DB 側に偏り、JSON Schema 戦略と思想が衝突する
6. **Repository レイヤ不要（NestJS の方針と整合）**
   - Drizzle 自体が Repository 風 API を提供し、Service が直接呼ぶ ADR 0014 の方針と素直に合致
...
| Prisma | スキーマ DSL ベースの ORM、Generated Client で型安全 | スキーマが独自 DSL（`.prisma`）で JSON Schema SSoT 戦略（[ADR 0006](./0006-json-schema-as-single-source-of-truth.md)）と相性が悪い。ランタイムが重い（Rust エンジン同梱）、`SKIP LOCKED` や `LISTEN/NOTIFY` を素直に書けず `$queryRaw` への退避が頻発する |
...
- **軽量ランタイム**：Prisma のような外部エンジンプロセス不要、コールドスタートが速い（ECS Fargate 最小タスク 1 構成と相性が良い、→ [ADR 0014](./0014-nestjs-for-backend.md)）
- **トランザクション内で混合操作可能**：`tx.insert().values()` と `tx.execute(sql\`NOTIFY ...\`)` を同一トランザクションで自然に書ける
- **Repository レイヤ不要**：Drizzle 自体が Repository 風の API を提供するため、Service が直接呼び出す方針（→ [ADR 0014](./0014-nestjs-for-backend.md)、[02-architecture.md: Backend API](../requirements/2-foundation/02-architecture.md#backend-apifastapi--python)）と整合
...
- [.claude/rules/alembic-sqlalchemy.md](../../.claude/rules/alembic-sqlalchemy.md)（Python pivot 後の ORM ルール、→ ADR 0037）
- [ADR 0004: Postgres をジョブキューに採用](./0004-postgres-as-job-queue.md)
- [ADR 0014: バックエンド API に NestJS を採用](./0014-nestjs-for-backend.md)
