...
- `commitlint`：PR の全コミット / push の HEAD を検証
- `Biome`：`pnpm lint`（lint + format 検証）
- `typecheck`：`pnpm typecheck`（Turborepo 経由で各 workspace の `tsc --noEmit`）

R0 では決めない（対応フェーズで追加）：

| 追加時期 | 項目 | 据え置く理由 |
|---|---|---|
| R1（API/Web 実装着手後） | `pnpm test`（Vitest 等）/ Drizzle schema 検証 | テストが書かれ始めてから接続する |
| R1〜R2 | Go の `gofmt` / `golangci-lint` / `go test` | grading-worker の実装開始後 |
| R2 | Docker build 検証（API・Worker のイメージビルド） | Dockerfile 確定後 |
| R2〜R3 | E2E（Playwright 等） | Web の主要画面が揃ってから |
| R3 | JSON Schema 生成物の drift チェック | `packages/shared-types` の運用開始後 |
| R3〜 | Terraform `fmt` / `validate` / `plan` | `infra/` 着手後 |
| 本番直前 | セキュリティスキャン（`pnpm audit` / Trivy / CodeQL）/ カバレッジ閾値 | 価値が出るのは資産が揃ってから |
| 本番運用 | デプロイワークフロー / リリースタグ自動化 | デプロイ先確定後 |
